"""Varnish House: cook-strain-flow outcome, sheet divergence, paint-house cross-effect."""
...
class VarnishHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_varnish_flowed_trace_replays(self) -> None:
        trace = _trace("marsh_varnish_flowed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("varnish_flowed", result.state.outcomes)
        self.assertEqual(result.state.location, "varn.flow")
        self.assertTrue(self.content.outcome_ready("varnish_flowed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_varn")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_varn")["actions"])
        self.assertEqual(marsh.state.location, "varn.yard")
        ...
        self.assertIn("know_the_cook", marsh_ids)
        self.assertNotIn("know_the_cook", city_ids)
        self.assertIn("read_the_varnish_list", city_ids)
        self.assertNotIn("read_the_varnish_list", marsh_ids)

    def test_coat_brushed_changes_cook_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_varn")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_paint_varn")["actions"])
        self.assertEqual(plain.state.location, "varn.cook")
        self.assertEqual(primed.state.location, "varn.cook")
        ...
        self.assertNotIn("prime_the_gum", plain_ids)
        self.assertIn("prime_the_gum", primed_ids)
        self.assertIn("coat_brushed", primed.state.outcomes)

    def test_player_plain_language_flows_varnish(self) -> None:
        ...
        self.assertIn("varnish_flowed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the varnish"):
    ...
    def test_i4_rejects_tampered_build_on_varnish_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_varnish_flowed"))
