"""Keelson House: scarph-pack-bolt outcome, sheet divergence, rider-house cross-effect."""
...
class KeelsonHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_sister_bolted_trace_replays(self) -> None:
        trace = _trace("marsh_sister_bolted")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("sister_bolted", result.state.outcomes)
        self.assertEqual(result.state.location, "ksn.sister")
        self.assertTrue(self.content.outcome_ready("sister_bolted", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_ksn")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_ksn")["actions"])
        self.assertEqual(marsh.state.location, "ksn.yard")
        ...
        self.assertIn("know_the_keelson", marsh_ids)
        self.assertNotIn("know_the_keelson", city_ids)
        self.assertIn("read_the_keelson_list", city_ids)
        self.assertNotIn("read_the_keelson_list", marsh_ids)

    def test_wring_dumped_changes_keelson_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_ksn")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_rid_ksn")["actions"])
        self.assertEqual(plain.state.location, "ksn.scarph")
        self.assertEqual(primed.state.location, "ksn.scarph")
        ...
        self.assertNotIn("wring_the_keelson", plain_ids)
        self.assertIn("wring_the_keelson", primed_ids)
        self.assertIn("wring_dumped", primed.state.outcomes)

    def test_player_plain_language_bolts_sister(self) -> None:
        ...
        self.assertIn("sister_bolted", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the keelson"):
    ...
    def test_i4_rejects_tampered_build_on_ksn_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_sister_bolted"))
