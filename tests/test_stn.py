"""Stanchion House: true-wedge-spike outcome, sheet divergence, pointer-house cross-effect."""
...
class StanchionHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_prior_outcomes_still_replay(self) -> None:
        for name, outcome in (
            ...
            ("marsh_parrel_trussed", "parrel_trussed"),
            ("marsh_cable_belayed", "cable_belayed"),
            ("marsh_gammon_frapped", "gammon_frapped"),
            ("marsh_tiller_yoked", "tiller_yoked"),
            ("marsh_fluke_fished", "fluke_fished"),
            ("marsh_stern_spiked", "stern_spiked"),
            ("marsh_mouth_plugged", "mouth_plugged"),
            ("marsh_card_locked", "card_locked"),
            ("marsh_belly_clenched", "belly_clenched"),
            ("marsh_bolster_seized", "bolster_seized"),
            ("marsh_waterway_paid", "waterway_paid"),
            ("marsh_coaming_coaked", "coaming_coaked"),
            ("marsh_bolt_dumped", "bolt_dumped"),
            ("marsh_davit_shipped", "davit_shipped"),
            ("marsh_guy_seized", "guy_seized"),
            ("marsh_strap_set", "strap_set"),
            ("marsh_kevel_clenched", "kevel_clenched"),
            ("marsh_knight_lashed", "knight_lashed"),
            ("marsh_carling_spiked", "carling_spiked"),
            ("marsh_keeper_pinned", "keeper_pinned"),
            ("marsh_edge_dumped", "edge_dumped"),
            ("marsh_spirket_clenched", "spirket_clenched"),
            ("marsh_bib_seized", "bib_seized"),
            ("marsh_hook_fayed", "hook_fayed"),
            ("marsh_lodging_nicked", "lodging_nicked"),
            ("marsh_sirmark_marked", "sirmark_marked"),
            ("marsh_wring_dumped", "wring_dumped"),
            ("marsh_sister_bolted", "sister_bolted"),
            ("marsh_crotch_spiked", "crotch_spiked"),
            ("marsh_compound_dumped", "compound_dumped"),
        ):
            trace = _trace(name)
            result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
            self.assertIn(outcome, result.state.outcomes, name)

    def test_foot_spiked_trace_replays(self) -> None:
        trace = _trace("marsh_foot_spiked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("foot_spiked", result.state.outcomes)
        self.assertEqual(result.state.location, "stn.foot")
        self.assertTrue(self.content.outcome_ready("foot_spiked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_stn")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_stn")["actions"])
        self.assertEqual(marsh.state.location, "stn.yard")
        ...
        self.assertIn("know_the_stanchion", marsh_ids)
        self.assertNotIn("know_the_stanchion", city_ids)
        self.assertIn("read_the_stanchion_list", city_ids)
        self.assertNotIn("read_the_stanchion_list", marsh_ids)

    def test_compound_dumped_changes_stanchion_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_stn")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_ptr_stn")["actions"])
        self.assertEqual(plain.state.location, "stn.true")
        self.assertEqual(primed.state.location, "stn.true")
        ...
        self.assertNotIn("dump_the_stanchion", plain_ids)
        self.assertIn("dump_the_stanchion", primed_ids)
        self.assertIn("compound_dumped", primed.state.outcomes)

    def test_player_plain_language_spikes_foot(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to the stanchions",
            "go to stanchion yard",
            "know the stanchion",
            "go to the true",
            "true the stanchion",
            "go to stanchion yard",
        ...
        self.assertIn("foot_spiked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the stanchions"):
    ...
    def test_i4_rejects_tampered_build_on_stn_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_foot_spiked"))
