"""Flax House: ret-heckle-spin outcome, sheet divergence, tannery cross-effect."""
...
class FlaxHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_flax_spun_trace_replays(self) -> None:
        trace = _trace("marsh_flax_spun")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("flax_spun", result.state.outcomes)
        self.assertEqual(result.state.location, "flax.spin")
        self.assertTrue(self.content.outcome_ready("flax_spun", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_flax")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_flax")["actions"])
        self.assertEqual(marsh.state.location, "flax.yard")
        ...
        self.assertIn("know_the_rett", marsh_ids)
        self.assertNotIn("know_the_rett", city_ids)
        self.assertIn("read_the_flax_list", city_ids)
        self.assertNotIn("read_the_flax_list", marsh_ids)

    def test_hide_tanned_changes_rett_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_flax")["actions"])
        gloved = replay(self.content, 1, "marsh_scout", _trace("cross_hide_flax")["actions"])
        self.assertEqual(plain.state.location, "flax.rett")
        self.assertEqual(gloved.state.location, "flax.rett")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        gloved_ids = {a.id for a in enumerate_legal(gloved.state, self.content)}
        self.assertNotIn("glove_the_rett", plain_ids)
        self.assertIn("glove_the_rett", gloved_ids)
        self.assertIn("hide_tanned", gloved.state.outcomes)

    def test_player_plain_language_spins_flax(self) -> None:
        ...
        self.assertIn("flax_spun", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the flax"):
    ...
    def test_i4_rejects_tampered_build_on_flax_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_flax_spun"))
