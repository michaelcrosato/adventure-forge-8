"""Glazier: score-groze-came outcome, sheet divergence, jeweler cross-effect."""
...
class GlazierTests(unittest.TestCase):
    @classmethod
    ...
    def test_pane_camed_trace_replays(self) -> None:
        trace = _trace("marsh_pane_camed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("pane_camed", result.state.outcomes)
        self.assertEqual(result.state.location, "glaz.came")
        self.assertTrue(self.content.outcome_ready("pane_camed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_glaz")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_glaz")["actions"])
        self.assertEqual(marsh.state.location, "glaz.yard")
        ...
        self.assertIn("know_the_score", marsh_ids)
        self.assertNotIn("know_the_score", city_ids)
        self.assertIn("read_the_glazier_list", city_ids)
        self.assertNotIn("read_the_glazier_list", marsh_ids)

    def test_collet_closed_changes_score_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_glaz")["actions"])
        gemmed = replay(self.content, 1, "marsh_scout", _trace("cross_gem_glaz")["actions"])
        self.assertEqual(plain.state.location, "glaz.score")
        self.assertEqual(gemmed.state.location, "glaz.score")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        gemmed_ids = {a.id for a in enumerate_legal(gemmed.state, self.content)}
        self.assertNotIn("gem_the_score", plain_ids)
        self.assertIn("gem_the_score", gemmed_ids)
        self.assertIn("collet_closed", gemmed.state.outcomes)

    def test_player_plain_language_cames_pane(self) -> None:
        ...
        self.assertIn("pane_camed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the glazier"):
    ...
    def test_i4_rejects_tampered_build_on_glaz_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_pane_camed"))
