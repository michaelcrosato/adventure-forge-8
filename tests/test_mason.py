"""Mason Yard: mix-lay-point outcome, sheet divergence, lime-kiln cross-effect."""
...
class MasonYardTests(unittest.TestCase):
    @classmethod
    ...
    def test_joint_pointed_trace_replays(self) -> None:
        trace = _trace("marsh_joint_pointed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("joint_pointed", result.state.outcomes)
        self.assertEqual(result.state.location, "mason.point")
        self.assertTrue(self.content.outcome_ready("joint_pointed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_mason")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_mason")["actions"])
        self.assertEqual(marsh.state.location, "mason.yard")
        ...
        self.assertIn("know_the_bed_joint", marsh_ids)
        self.assertNotIn("know_the_bed_joint", city_ids)
        self.assertIn("read_the_mason_list", city_ids)
        self.assertNotIn("read_the_mason_list", marsh_ids)

    def test_lime_slaked_changes_mix_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_mason")["actions"])
        tempered = replay(self.content, 1, "marsh_scout", _trace("cross_lime_mason")["actions"])
        self.assertEqual(plain.state.location, "mason.mix")
        self.assertEqual(tempered.state.location, "mason.mix")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        tempered_ids = {a.id for a in enumerate_legal(tempered.state, self.content)}
        self.assertNotIn("temper_the_lime", plain_ids)
        self.assertIn("temper_the_lime", tempered_ids)
        self.assertIn("lime_slaked", tempered.state.outcomes)

    def test_player_plain_language_points_joint(self) -> None:
        ...
        self.assertIn("joint_pointed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the masons"):
    ...
    def test_i4_rejects_tampered_build_on_mason_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_joint_pointed"))
