"""Clinker Shed: steam-clench-fair outcome, sheet divergence, launch-ways cross-effect."""
...
class ClinkerShedTests(unittest.TestCase):
    @classmethod
    ...
    def test_garboard_faired_trace_replays(self) -> None:
        trace = _trace("marsh_garboard_faired")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("garboard_faired", result.state.outcomes)
        self.assertEqual(result.state.location, "clink.garboard")
        self.assertTrue(self.content.outcome_ready("garboard_faired", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_clink")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_clink")["actions"])
        self.assertEqual(marsh.state.location, "clink.yard")
        ...
        self.assertIn("know_the_steam", marsh_ids)
        self.assertNotIn("know_the_steam", city_ids)
        self.assertIn("read_the_clinker_list", city_ids)
        self.assertNotIn("read_the_clinker_list", marsh_ids)

    def test_hull_launched_changes_steam_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_clink")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_ways_clink")["actions"])
        self.assertEqual(plain.state.location, "clink.steam")
        self.assertEqual(primed.state.location, "clink.steam")
        ...
        self.assertNotIn("launch_the_steam", plain_ids)
        self.assertIn("launch_the_steam", primed_ids)
        self.assertIn("hull_launched", primed.state.outcomes)

    def test_player_plain_language_fairs_garboard(self) -> None:
        ...
        self.assertIn("garboard_faired", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the clinker"):
    ...
    def test_i4_rejects_tampered_build_on_clink_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_garboard_faired"))
