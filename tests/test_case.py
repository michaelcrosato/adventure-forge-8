"""Casing House: mitre-scribe-tack outcome, sheet divergence, sill-house cross-effect."""
...
class CasingHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_casing_tacked_trace_replays(self) -> None:
        trace = _trace("marsh_casing_tacked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("casing_tacked", result.state.outcomes)
        self.assertEqual(result.state.location, "case.tack")
        self.assertTrue(self.content.outcome_ready("casing_tacked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_case")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_case")["actions"])
        self.assertEqual(marsh.state.location, "case.yard")
        ...
        self.assertIn("know_the_mitre", marsh_ids)
        self.assertNotIn("know_the_mitre", city_ids)
        self.assertIn("read_the_casing_list", city_ids)
        self.assertNotIn("read_the_casing_list", marsh_ids)

    def test_stool_seated_changes_mitre_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_case")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_sill_case")["actions"])
        self.assertEqual(plain.state.location, "case.mitre")
        self.assertEqual(primed.state.location, "case.mitre")
        ...
        self.assertNotIn("stool_the_mitre", plain_ids)
        self.assertIn("stool_the_mitre", primed_ids)
        self.assertIn("stool_seated", primed.state.outcomes)

    def test_player_plain_language_tacks_casing(self) -> None:
        ...
        self.assertIn("casing_tacked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the casing"):
    ...
    def test_i4_rejects_tampered_build_on_casing_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_casing_tacked"))
