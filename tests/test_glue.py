"""Glue House: trim-seethe-cake outcome, sheet divergence, vinegar-house cross-effect."""
...
class GlueHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_glue_caked_trace_replays(self) -> None:
        trace = _trace("marsh_glue_caked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("glue_caked", result.state.outcomes)
        self.assertEqual(result.state.location, "glue.slab")
        self.assertTrue(self.content.outcome_ready("glue_caked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_glue")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_glue")["actions"])
        self.assertEqual(marsh.state.location, "glue.yard")
        ...
        self.assertIn("know_the_paring", marsh_ids)
        self.assertNotIn("know_the_paring", city_ids)
        self.assertIn("read_the_glue_list", city_ids)
        self.assertNotIn("read_the_glue_list", marsh_ids)

    def test_cruet_corked_changes_paring_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_glue")["actions"])
        aceted = replay(self.content, 1, "marsh_scout", _trace("cross_acet_glue")["actions"])
        self.assertEqual(plain.state.location, "glue.paring")
        self.assertEqual(aceted.state.location, "glue.paring")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        aceted_ids = {a.id for a in enumerate_legal(aceted.state, self.content)}
        self.assertNotIn("acet_the_paring", plain_ids)
        self.assertIn("acet_the_paring", aceted_ids)
        self.assertIn("cruet_corked", aceted.state.outcomes)

    def test_player_plain_language_cakes_glue(self) -> None:
        ...
        self.assertIn("glue_caked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the glue"):
    ...
    def test_i4_rejects_tampered_build_on_glue_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_glue_caked"))
