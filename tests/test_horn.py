"""Horn Lantern: scrape-set-hang outcome, sheet divergence, loom-shed cross-effect."""
...
class HornLanternTests(unittest.TestCase):
    @classmethod
    ...
    def test_lantern_hung_trace_replays(self) -> None:
        trace = _trace("marsh_lantern_hung")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("lantern_hung", result.state.outcomes)
        self.assertEqual(result.state.location, "horn.hook")
        self.assertTrue(self.content.outcome_ready("lantern_hung", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_horn")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_horn")["actions"])
        self.assertEqual(marsh.state.location, "horn.yard")
        ...
        self.assertIn("know_the_horn", marsh_ids)
        self.assertNotIn("know_the_horn", city_ids)
        self.assertIn("read_the_lantern_list", city_ids)
        self.assertNotIn("read_the_lantern_list", marsh_ids)

    def test_web_sheared_changes_scrape_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_horn")["actions"])
        wicked = replay(self.content, 1, "marsh_scout", _trace("cross_loom_horn")["actions"])
        self.assertEqual(plain.state.location, "horn.scrape")
        self.assertEqual(wicked.state.location, "horn.scrape")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        wicked_ids = {a.id for a in enumerate_legal(wicked.state, self.content)}
        self.assertNotIn("wick_the_horn", plain_ids)
        self.assertIn("wick_the_horn", wicked_ids)
        self.assertIn("web_sheared", wicked.state.outcomes)

    def test_player_plain_language_hangs_lantern(self) -> None:
        ...
        self.assertIn("lantern_hung", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the lanterns"):
    ...
    def test_i4_rejects_tampered_build_on_horn_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_lantern_hung"))
