"""Hawse House: bore-seat-pay outcome, sheet divergence, cleat-house cross-effect."""
...
class HawseHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_collar_paid_trace_replays(self) -> None:
        trace = _trace("marsh_collar_paid")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("collar_paid", result.state.outcomes)
        self.assertEqual(result.state.location, "haw.collar")
        self.assertTrue(self.content.outcome_ready("collar_paid", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_haw")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_haw")["actions"])
        self.assertEqual(marsh.state.location, "haw.yard")
        ...
        self.assertIn("know_the_bore", marsh_ids)
        self.assertNotIn("know_the_bore", city_ids)
        self.assertIn("read_the_hawse_list", city_ids)
        self.assertNotIn("read_the_hawse_list", marsh_ids)

    def test_base_bolted_changes_bore_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_haw")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_cleat_haw")["actions"])
        self.assertEqual(plain.state.location, "haw.bore")
        self.assertEqual(primed.state.location, "haw.bore")
        ...
        self.assertNotIn("bolt_the_bore", plain_ids)
        self.assertIn("bolt_the_bore", primed_ids)
        self.assertIn("base_bolted", primed.state.outcomes)

    def test_player_plain_language_pays_collar(self) -> None:
        ...
        self.assertIn("collar_paid", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the hawse"):
    ...
    def test_i4_rejects_tampered_build_on_haw_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_collar_paid"))
