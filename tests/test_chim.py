"""Chimney House: bed-set-lime outcome, sheet divergence, lath-house cross-effect."""
...
class ChimneyHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_breast_limed_trace_replays(self) -> None:
        trace = _trace("marsh_breast_limed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("breast_limed", result.state.outcomes)
        self.assertEqual(result.state.location, "chim.breast")
        self.assertTrue(self.content.outcome_ready("breast_limed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_chim")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_chim")["actions"])
        self.assertEqual(marsh.state.location, "chim.yard")
        ...
        self.assertIn("know_the_flag", marsh_ids)
        self.assertNotIn("know_the_flag", city_ids)
        self.assertIn("read_the_chimney_list", city_ids)
        self.assertNotIn("read_the_chimney_list", marsh_ids)

    def test_coat_haired_changes_flag_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_chim")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_lath_chim")["actions"])
        self.assertEqual(plain.state.location, "chim.flag")
        self.assertEqual(primed.state.location, "chim.flag")
        ...
        self.assertNotIn("hair_the_flag", plain_ids)
        self.assertIn("hair_the_flag", primed_ids)
        self.assertIn("coat_haired", primed.state.outcomes)

    def test_player_plain_language_limes_breast(self) -> None:
        ...
        self.assertIn("breast_limed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the chimney"):
    ...
    def test_i4_rejects_tampered_build_on_chim_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_breast_limed"))
