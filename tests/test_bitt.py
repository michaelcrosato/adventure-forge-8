"""Bitts House: step-cross-belay outcome, sheet divergence, parrel-house cross-effect."""
...
class BittsHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_cable_belayed_trace_replays(self) -> None:
        trace = _trace("marsh_cable_belayed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("cable_belayed", result.state.outcomes)
        self.assertEqual(result.state.location, "bitt.belay")
        self.assertTrue(self.content.outcome_ready("cable_belayed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_bitt")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_bitt")["actions"])
        self.assertEqual(marsh.state.location, "bitt.yard")
        ...
        self.assertIn("know_the_step", marsh_ids)
        self.assertNotIn("know_the_step", city_ids)
        self.assertIn("read_the_bitts_list", city_ids)
        self.assertNotIn("read_the_bitts_list", marsh_ids)

    def test_parrel_trussed_changes_step_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_bitt")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_par_bitt")["actions"])
        self.assertEqual(plain.state.location, "bitt.step")
        self.assertEqual(primed.state.location, "bitt.step")
        ...
        self.assertNotIn("truss_the_step", plain_ids)
        self.assertIn("truss_the_step", primed_ids)
        self.assertIn("parrel_trussed", primed.state.outcomes)

    def test_player_plain_language_belays_cable(self) -> None:
        ...
        self.assertIn("cable_belayed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the bitts"):
    ...
    def test_i4_rejects_tampered_build_on_bitt_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_cable_belayed"))
