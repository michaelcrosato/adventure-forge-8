"""Scupper House: broach-chase-plug outcome, sheet divergence, transom-house cross-effect."""
...
class ScupperHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_mouth_plugged_trace_replays(self) -> None:
        trace = _trace("marsh_mouth_plugged")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("mouth_plugged", result.state.outcomes)
        self.assertEqual(result.state.location, "scup.mouth")
        self.assertTrue(self.content.outcome_ready("mouth_plugged", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_scup")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_scup")["actions"])
        self.assertEqual(marsh.state.location, "scup.yard")
        ...
        self.assertIn("know_the_broach", marsh_ids)
        self.assertNotIn("know_the_broach", city_ids)
        self.assertIn("read_the_scupper_list", city_ids)
        self.assertNotIn("read_the_scupper_list", marsh_ids)

    def test_stern_spiked_changes_broach_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_scup")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_tran_scup")["actions"])
        self.assertEqual(plain.state.location, "scup.broach")
        self.assertEqual(primed.state.location, "scup.broach")
        ...
        self.assertNotIn("spike_the_broach", plain_ids)
        self.assertIn("spike_the_broach", primed_ids)
        self.assertIn("stern_spiked", primed.state.outcomes)

    def test_player_plain_language_plugs_mouth(self) -> None:
        ...
        self.assertIn("mouth_plugged", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the scupper"):
    ...
    def test_i4_rejects_tampered_build_on_scup_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_mouth_plugged"))
