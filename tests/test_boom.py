"""Boomkin House: steeve-bee-seize outcome, sheet divergence, davit-house cross-effect."""
...
class BoomkinHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_guy_seized_trace_replays(self) -> None:
        trace = _trace("marsh_guy_seized")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("guy_seized", result.state.outcomes)
        self.assertEqual(result.state.location, "boom.guy")
        self.assertTrue(self.content.outcome_ready("guy_seized", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_boom")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_boom")["actions"])
        self.assertEqual(marsh.state.location, "boom.yard")
        ...
        self.assertIn("know_the_steeve", marsh_ids)
        self.assertNotIn("know_the_steeve", city_ids)
        self.assertIn("read_the_boomkin_list", city_ids)
        self.assertNotIn("read_the_boomkin_list", marsh_ids)

    def test_davit_shipped_changes_steeve_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_boom")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_dav_boom")["actions"])
        self.assertEqual(plain.state.location, "boom.steeve")
        self.assertEqual(primed.state.location, "boom.steeve")
        ...
        self.assertNotIn("ship_the_steeve", plain_ids)
        self.assertIn("ship_the_steeve", primed_ids)
        self.assertIn("davit_shipped", primed.state.outcomes)

    def test_player_plain_language_seizes_guy(self) -> None:
        ...
        self.assertIn("guy_seized", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the boomkin"):
    ...
    def test_i4_rejects_tampered_build_on_boom_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_guy_seized"))
