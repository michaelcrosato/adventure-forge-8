"""Dado House: plough-house-cap outcome, sheet divergence, skirting-house cross-effect."""
...
class DadoHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_rail_capped_trace_replays(self) -> None:
        trace = _trace("marsh_rail_capped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("rail_capped", result.state.outcomes)
        self.assertEqual(result.state.location, "dado.cap")
        self.assertTrue(self.content.outcome_ready("rail_capped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_dado")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_dado")["actions"])
        self.assertEqual(marsh.state.location, "dado.yard")
        ...
        self.assertIn("know_the_plough", marsh_ids)
        self.assertNotIn("know_the_plough", city_ids)
        self.assertIn("read_the_dado_list", city_ids)
        self.assertNotIn("read_the_dado_list", marsh_ids)

    def test_plinth_fixed_changes_plough_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_dado")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_skirt_dado")["actions"])
        self.assertEqual(plain.state.location, "dado.plough")
        self.assertEqual(primed.state.location, "dado.plough")
        ...
        self.assertNotIn("plinth_the_dado", plain_ids)
        self.assertIn("plinth_the_dado", primed_ids)
        self.assertIn("plinth_fixed", primed.state.outcomes)

    def test_player_plain_language_caps_rail(self) -> None:
        ...
        self.assertIn("rail_capped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the dado"):
    ...
    def test_i4_rejects_tampered_build_on_dado_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_rail_capped"))
