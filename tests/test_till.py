"""Tiller House: shape-fit-yoke outcome, sheet divergence, gammoning-house cross-effect."""
...
class TillerHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_tiller_yoked_trace_replays(self) -> None:
        trace = _trace("marsh_tiller_yoked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("tiller_yoked", result.state.outcomes)
        self.assertEqual(result.state.location, "till.yoke")
        self.assertTrue(self.content.outcome_ready("tiller_yoked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_till")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_till")["actions"])
        self.assertEqual(marsh.state.location, "till.yard")
        ...
        self.assertIn("know_the_shape", marsh_ids)
        self.assertNotIn("know_the_shape", city_ids)
        self.assertIn("read_the_tiller_list", city_ids)
        self.assertNotIn("read_the_tiller_list", marsh_ids)

    def test_gammon_frapped_changes_shape_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_till")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_gam_till")["actions"])
        self.assertEqual(plain.state.location, "till.shape")
        self.assertEqual(primed.state.location, "till.shape")
        ...
        self.assertNotIn("frap_the_shape", plain_ids)
        self.assertIn("frap_the_shape", primed_ids)
        self.assertIn("gammon_frapped", primed.state.outcomes)

    def test_player_plain_language_yokes_tiller(self) -> None:
        ...
        self.assertIn("tiller_yoked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the tiller"):
    ...
    def test_i4_rejects_tampered_build_on_till_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_tiller_yoked"))
