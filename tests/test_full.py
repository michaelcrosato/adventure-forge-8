"""Fulling Mill: wet-walk-hang outcome, sheet divergence, soap-house cross-effect."""
...
class FullingMillTests(unittest.TestCase):
    @classmethod
    ...
    def test_cloth_fulled_trace_replays(self) -> None:
        trace = _trace("marsh_cloth_fulled")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("cloth_fulled", result.state.outcomes)
        self.assertEqual(result.state.location, "full.tenters")
        self.assertTrue(self.content.outcome_ready("cloth_fulled", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_full")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_full")["actions"])
        self.assertEqual(marsh.state.location, "full.yard")
        ...
        self.assertIn("know_the_full_walk", marsh_ids)
        self.assertNotIn("know_the_full_walk", city_ids)
        self.assertIn("read_the_full_list", city_ids)
        self.assertNotIn("read_the_full_list", marsh_ids)

    def test_soap_cut_changes_web_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_full")["actions"])
        soaped = replay(self.content, 1, "marsh_scout", _trace("cross_soap_full")["actions"])
        self.assertEqual(plain.state.location, "full.web")
        self.assertEqual(soaped.state.location, "full.web")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        soaped_ids = {a.id for a in enumerate_legal(soaped.state, self.content)}
        self.assertNotIn("soap_the_web", plain_ids)
        self.assertIn("soap_the_web", soaped_ids)
        self.assertIn("soap_cut", soaped.state.outcomes)

    def test_player_plain_language_fulls_cloth(self) -> None:
        ...
        self.assertIn("cloth_fulled", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the fulling"):
    ...
    def test_i4_rejects_tampered_build_on_full_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_cloth_fulled"))
