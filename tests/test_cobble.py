"""Cobble Shop: last-awl-peg outcome, sheet divergence, gall-house cross-effect."""
...
class CobbleShopTests(unittest.TestCase):
    @classmethod
    ...
    def test_heel_pegged_trace_replays(self) -> None:
        trace = _trace("marsh_heel_pegged")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("heel_pegged", result.state.outcomes)
        self.assertEqual(result.state.location, "cobble.heel")
        self.assertTrue(self.content.outcome_ready("heel_pegged", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_cobble")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_cobble")["actions"])
        self.assertEqual(marsh.state.location, "cobble.yard")
        ...
        self.assertIn("know_the_last", marsh_ids)
        self.assertNotIn("know_the_last", city_ids)
        self.assertIn("read_the_cobble_list", city_ids)
        self.assertNotIn("read_the_cobble_list", marsh_ids)

    def test_nib_cut_changes_sole_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_cobble")["actions"])
        marked = replay(self.content, 1, "marsh_scout", _trace("cross_gall_cobble")["actions"])
        self.assertEqual(plain.state.location, "cobble.sole")
        self.assertEqual(marked.state.location, "cobble.sole")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        marked_ids = {a.id for a in enumerate_legal(marked.state, self.content)}
        self.assertNotIn("mark_the_last", plain_ids)
        self.assertIn("mark_the_last", marked_ids)
        self.assertIn("nib_cut", marked.state.outcomes)

    def test_player_plain_language_pegs_heel(self) -> None:
        ...
        self.assertIn("heel_pegged", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the cobblers"):
    ...
    def test_i4_rejects_tampered_build_on_cobble_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_heel_pegged"))
