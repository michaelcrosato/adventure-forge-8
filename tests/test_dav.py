"""Davit House: sweep-band-ship outcome, sheet divergence, gunwale-house cross-effect."""
...
class DavitHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_davit_shipped_trace_replays(self) -> None:
        trace = _trace("marsh_davit_shipped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("davit_shipped", result.state.outcomes)
        self.assertEqual(result.state.location, "dav.ship")
        self.assertTrue(self.content.outcome_ready("davit_shipped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_dav")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_dav")["actions"])
        self.assertEqual(marsh.state.location, "dav.yard")
        ...
        self.assertIn("know_the_sweep", marsh_ids)
        self.assertNotIn("know_the_sweep", city_ids)
        self.assertIn("read_the_davit_list", city_ids)
        self.assertNotIn("read_the_davit_list", marsh_ids)

    def test_bolt_dumped_changes_sweep_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_dav")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_wale_dav")["actions"])
        self.assertEqual(plain.state.location, "dav.sweep")
        self.assertEqual(primed.state.location, "dav.sweep")
        ...
        self.assertNotIn("dump_the_sweep", plain_ids)
        self.assertIn("dump_the_sweep", primed_ids)
        self.assertIn("bolt_dumped", primed.state.outcomes)

    def test_player_plain_language_ships_davit(self) -> None:
        ...
        self.assertIn("davit_shipped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the davit"):
    ...
    def test_i4_rejects_tampered_build_on_dav_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_davit_shipped"))
