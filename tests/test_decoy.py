"""Decoy Pond: pipe-drive-bag outcome, sheet divergence, tide-forge cross-effect."""
...
class DecoyPondTests(unittest.TestCase):
    @classmethod
    ...
    def test_fowl_taken_trace_replays(self) -> None:
        trace = _trace("marsh_fowl_taken")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("fowl_taken", result.state.outcomes)
        self.assertEqual(result.state.location, "decoy.tunnel")
        self.assertTrue(self.content.outcome_ready("fowl_taken", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_decoy")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_decoy")["actions"])
        self.assertEqual(marsh.state.location, "decoy.yard")
        ...
        self.assertIn("know_the_decoy_run", marsh_ids)
        self.assertNotIn("know_the_decoy_run", city_ids)
        self.assertIn("read_the_decoy_list", city_ids)
        self.assertNotIn("read_the_decoy_list", marsh_ids)

    def test_iron_quenched_changes_tunnel_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_decoy")["actions"])
        hooked = replay(self.content, 1, "marsh_scout", _trace("cross_forge_decoy")["actions"])
        self.assertEqual(plain.state.location, "decoy.tunnel")
        self.assertEqual(hooked.state.location, "decoy.tunnel")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        hooked_ids = {a.id for a in enumerate_legal(hooked.state, self.content)}
        self.assertNotIn("hook_the_take", plain_ids)
        self.assertIn("hook_the_take", hooked_ids)
        self.assertIn("iron_quenched", hooked.state.outcomes)

    def test_player_plain_language_bags_fowl(self) -> None:
        ...
        self.assertIn("fowl_taken", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to decoy pond"):
    ...
    def test_i4_rejects_tampered_build_on_decoy_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_fowl_taken"))
