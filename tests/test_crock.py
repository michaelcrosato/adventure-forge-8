"""Crock Yard: throw-fire-glaze outcome, sheet divergence, jam-house cross-effect."""
...
class CrockYardTests(unittest.TestCase):
    @classmethod
    ...
    def test_crock_glazed_trace_replays(self) -> None:
        trace = _trace("marsh_crock_glazed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("crock_glazed", result.state.outcomes)
        self.assertEqual(result.state.location, "crock.glaze")
        self.assertTrue(self.content.outcome_ready("crock_glazed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_crock")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_crock")["actions"])
        self.assertEqual(marsh.state.location, "crock.yard")
        ...
        self.assertIn("know_the_throw", marsh_ids)
        self.assertNotIn("know_the_throw", city_ids)
        self.assertIn("read_the_crock_list", city_ids)
        self.assertNotIn("read_the_crock_list", marsh_ids)

    def test_jam_jarred_changes_throw_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_crock")["actions"])
        fluxed = replay(self.content, 1, "marsh_scout", _trace("cross_jam_crock")["actions"])
        self.assertEqual(plain.state.location, "crock.throw")
        self.assertEqual(fluxed.state.location, "crock.throw")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        fluxed_ids = {a.id for a in enumerate_legal(fluxed.state, self.content)}
        self.assertNotIn("jam_the_flux", plain_ids)
        self.assertIn("jam_the_flux", fluxed_ids)
        self.assertIn("jam_jarred", fluxed.state.outcomes)

    def test_player_plain_language_glazes_crock(self) -> None:
        ...
        self.assertIn("crock_glazed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the crocks"):
    ...
    def test_i4_rejects_tampered_build_on_crock_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_crock_glazed"))
