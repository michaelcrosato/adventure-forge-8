"""Picture Rail: snap-plug-spring outcome, sheet divergence, dado-house cross-effect."""
...
class PictureRailTests(unittest.TestCase):
    @classmethod
    ...
    def test_rail_sprung_trace_replays(self) -> None:
        trace = _trace("marsh_rail_sprung")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("rail_sprung", result.state.outcomes)
        self.assertEqual(result.state.location, "pic.spring")
        self.assertTrue(self.content.outcome_ready("rail_sprung", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_pic")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_pic")["actions"])
        self.assertEqual(marsh.state.location, "pic.yard")
        ...
        self.assertIn("know_the_chalk", marsh_ids)
        self.assertNotIn("know_the_chalk", city_ids)
        self.assertIn("read_the_picture_list", city_ids)
        self.assertNotIn("read_the_picture_list", marsh_ids)

    def test_rail_capped_changes_chalk_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_pic")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_dado_pic")["actions"])
        self.assertEqual(plain.state.location, "pic.chalk")
        self.assertEqual(primed.state.location, "pic.chalk")
        ...
        self.assertNotIn("rail_the_chalk", plain_ids)
        self.assertIn("rail_the_chalk", primed_ids)
        self.assertIn("rail_capped", primed.state.outcomes)

    def test_player_plain_language_springs_rail(self) -> None:
        ...
        self.assertIn("rail_sprung", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the pictures"):
    ...
    def test_i4_rejects_tampered_build_on_picture_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_rail_sprung"))
