"""Tannery: flesh-bate-tan outcome, sheet divergence, crock-yard cross-effect."""
...
class TanneryTests(unittest.TestCase):
    @classmethod
    ...
    def test_hide_tanned_trace_replays(self) -> None:
        trace = _trace("marsh_hide_tanned")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("hide_tanned", result.state.outcomes)
        self.assertEqual(result.state.location, "hide.tan")
        self.assertTrue(self.content.outcome_ready("hide_tanned", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_hide")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_hide")["actions"])
        self.assertEqual(marsh.state.location, "hide.yard")
        ...
        self.assertIn("know_the_flesh", marsh_ids)
        self.assertNotIn("know_the_flesh", city_ids)
        self.assertIn("read_the_hide_list", city_ids)
        self.assertNotIn("read_the_hide_list", marsh_ids)

    def test_crock_glazed_changes_flesh_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_hide")["actions"])
        rinsed = replay(self.content, 1, "marsh_scout", _trace("cross_crock_hide")["actions"])
        self.assertEqual(plain.state.location, "hide.flesh")
        self.assertEqual(rinsed.state.location, "hide.flesh")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        rinsed_ids = {a.id for a in enumerate_legal(rinsed.state, self.content)}
        self.assertNotIn("crock_the_rinse", plain_ids)
        self.assertIn("crock_the_rinse", rinsed_ids)
        self.assertIn("crock_glazed", rinsed.state.outcomes)

    def test_player_plain_language_tans_hide(self) -> None:
        ...
        self.assertIn("hide_tanned", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the hides"):
    ...
    def test_i4_rejects_tampered_build_on_hide_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_hide_tanned"))
