"""Carling House: notch-lodge-spike outcome, sheet divergence, knighthead-house cross-effect."""
...
class CarlingHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_carling_spiked_trace_replays(self) -> None:
        trace = _trace("marsh_carling_spiked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("carling_spiked", result.state.outcomes)
        self.assertEqual(result.state.location, "carl.spike")
        self.assertTrue(self.content.outcome_ready("carling_spiked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_carl")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_carl")["actions"])
        self.assertEqual(marsh.state.location, "carl.yard")
        ...
        self.assertIn("know_the_notch", marsh_ids)
        self.assertNotIn("know_the_notch", city_ids)
        self.assertIn("read_the_carling_list", city_ids)
        self.assertNotIn("read_the_carling_list", marsh_ids)

    def test_knight_lashed_changes_notch_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_carl")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_kni_carl")["actions"])
        self.assertEqual(plain.state.location, "carl.notch")
        self.assertEqual(primed.state.location, "carl.notch")
        ...
        self.assertNotIn("lash_the_notch", plain_ids)
        self.assertIn("lash_the_notch", primed_ids)
        self.assertIn("knight_lashed", primed.state.outcomes)

    def test_player_plain_language_spikes_carling(self) -> None:
        ...
        self.assertIn("carling_spiked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the carling"):
    ...
    def test_i4_rejects_tampered_build_on_carl_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_carling_spiked"))
