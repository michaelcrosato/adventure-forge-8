"""Windlass House: ship-pawl-heave outcome, sheet divergence, clinker-shed cross-effect."""
...
class WindlassHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_round_heaved_trace_replays(self) -> None:
        trace = _trace("marsh_round_heaved")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("round_heaved", result.state.outcomes)
        self.assertEqual(result.state.location, "wind.drum")
        self.assertTrue(self.content.outcome_ready("round_heaved", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_wind")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_wind")["actions"])
        self.assertEqual(marsh.state.location, "wind.yard")
        ...
        self.assertIn("know_the_bars", marsh_ids)
        self.assertNotIn("know_the_bars", city_ids)
        self.assertIn("read_the_windlass_list", city_ids)
        self.assertNotIn("read_the_windlass_list", marsh_ids)

    def test_garboard_faired_changes_bars_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_wind")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_clink_wind")["actions"])
        self.assertEqual(plain.state.location, "wind.bars")
        self.assertEqual(primed.state.location, "wind.bars")
        ...
        self.assertNotIn("fair_the_bars", plain_ids)
        self.assertIn("fair_the_bars", primed_ids)
        self.assertIn("garboard_faired", primed.state.outcomes)

    def test_player_plain_language_heaves_round(self) -> None:
        ...
        self.assertIn("round_heaved", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the windlass"):
    ...
    def test_i4_rejects_tampered_build_on_wind_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_round_heaved"))
