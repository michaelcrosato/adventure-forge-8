"""Cornice House: run-key-float outcome, sheet divergence, picture-rail cross-effect."""
...
class CorniceHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_cornice_floated_trace_replays(self) -> None:
        trace = _trace("marsh_cornice_floated")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("cornice_floated", result.state.outcomes)
        self.assertEqual(result.state.location, "corn.float")
        self.assertTrue(self.content.outcome_ready("cornice_floated", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_corn")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_corn")["actions"])
        self.assertEqual(marsh.state.location, "corn.yard")
        ...
        self.assertIn("know_the_cove", marsh_ids)
        self.assertNotIn("know_the_cove", city_ids)
        self.assertIn("read_the_cornice_list", city_ids)
        self.assertNotIn("read_the_cornice_list", marsh_ids)

    def test_rail_sprung_changes_cove_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_corn")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_pic_corn")["actions"])
        self.assertEqual(plain.state.location, "corn.cove")
        self.assertEqual(primed.state.location, "corn.cove")
        ...
        self.assertNotIn("spring_the_cove", plain_ids)
        self.assertIn("spring_the_cove", primed_ids)
        self.assertIn("rail_sprung", primed.state.outcomes)

    def test_player_plain_language_floats_cornice(self) -> None:
        ...
        self.assertIn("cornice_floated", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the cornice"):
    ...
    def test_i4_rejects_tampered_build_on_cornice_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_cornice_floated"))
