"""Flue House: parge-wad-hang outcome, sheet divergence, mantel-house cross-effect."""
...
class FlueHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_cowl_hung_trace_replays(self) -> None:
        trace = _trace("marsh_cowl_hung")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("cowl_hung", result.state.outcomes)
        self.assertEqual(result.state.location, "flue.cowl")
        self.assertTrue(self.content.outcome_ready("cowl_hung", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_flue")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_flue")["actions"])
        self.assertEqual(marsh.state.location, "flue.yard")
        ...
        self.assertIn("know_the_parge", marsh_ids)
        self.assertNotIn("know_the_parge", city_ids)
        self.assertIn("read_the_flue_list", city_ids)
        self.assertNotIn("read_the_flue_list", marsh_ids)

    def test_mantel_pinned_changes_parge_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_flue")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_mant_flue")["actions"])
        self.assertEqual(plain.state.location, "flue.parge")
        self.assertEqual(primed.state.location, "flue.parge")
        ...
        self.assertNotIn("pin_the_parge", plain_ids)
        self.assertIn("pin_the_parge", primed_ids)
        self.assertIn("mantel_pinned", primed.state.outcomes)

    def test_player_plain_language_hangs_cowl(self) -> None:
        ...
        self.assertIn("cowl_hung", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the flues"):
    ...
    def test_i4_rejects_tampered_build_on_flue_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_cowl_hung"))
