"""Brick Yard: pug-strike-hack outcome, sheet divergence, grate-house cross-effect."""
...
class BrickYardTests(unittest.TestCase):
    @classmethod
    ...
    def test_hack_set_trace_replays(self) -> None:
        trace = _trace("marsh_hack_set")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("hack_set", result.state.outcomes)
        self.assertEqual(result.state.location, "brick.hack")
        self.assertTrue(self.content.outcome_ready("hack_set", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_brick")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_brick")["actions"])
        self.assertEqual(marsh.state.location, "brick.yard")
        ...
        self.assertIn("know_the_pug", marsh_ids)
        self.assertNotIn("know_the_pug", city_ids)
        self.assertIn("read_the_brick_list", city_ids)
        self.assertNotIn("read_the_brick_list", marsh_ids)

    def test_slide_registered_changes_pug_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_brick")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_grate_brick")["actions"])
        self.assertEqual(plain.state.location, "brick.pug")
        self.assertEqual(primed.state.location, "brick.pug")
        ...
        self.assertNotIn("grate_the_pug", plain_ids)
        self.assertIn("grate_the_pug", primed_ids)
        self.assertIn("slide_registered", primed.state.outcomes)

    def test_player_plain_language_sets_hack(self) -> None:
        ...
        self.assertIn("hack_set", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the bricks"):
    ...
    def test_i4_rejects_tampered_build_on_brick_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_hack_set"))
