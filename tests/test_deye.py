"""Deadeye Loft: score-reeve-seize outcome, sheet divergence, hawse-house cross-effect."""
...
class DeadeyeLoftTests(unittest.TestCase):
    @classmethod
    ...
    def test_lanyard_seized_trace_replays(self) -> None:
        trace = _trace("marsh_lanyard_seized")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("lanyard_seized", result.state.outcomes)
        self.assertEqual(result.state.location, "deye.lanyard")
        self.assertTrue(self.content.outcome_ready("lanyard_seized", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_deye")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_deye")["actions"])
        self.assertEqual(marsh.state.location, "deye.yard")
        ...
        self.assertIn("know_the_rim", marsh_ids)
        self.assertNotIn("know_the_rim", city_ids)
        self.assertIn("read_the_deadeye_list", city_ids)
        self.assertNotIn("read_the_deadeye_list", marsh_ids)

    def test_collar_paid_changes_rim_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_deye")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_haw_deye")["actions"])
        self.assertEqual(plain.state.location, "deye.rim")
        self.assertEqual(primed.state.location, "deye.rim")
        ...
        self.assertNotIn("pay_the_rim", plain_ids)
        self.assertIn("pay_the_rim", primed_ids)
        self.assertIn("collar_paid", primed.state.outcomes)

    def test_player_plain_language_seizes_lanyard(self) -> None:
        ...
        self.assertIn("lanyard_seized", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the deadeye"):
    ...
    def test_i4_rejects_tampered_build_on_deye_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_lanyard_seized"))
