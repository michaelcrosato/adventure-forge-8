"""Sail Loft: cut-stitch-hoist outcome, sheet divergence, net-loft cross-effect."""
...
class SailLoftTests(unittest.TestCase):
    @classmethod
    ...
    def test_sail_hoisted_trace_replays(self) -> None:
        trace = _trace("marsh_sail_hoisted")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("sail_hoisted", result.state.outcomes)
        self.assertEqual(result.state.location, "sail.hoist")
        self.assertTrue(self.content.outcome_ready("sail_hoisted", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_sail")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_sail")["actions"])
        self.assertEqual(marsh.state.location, "sail.yard")
        ...
        self.assertIn("know_the_canvas_cut", marsh_ids)
        self.assertNotIn("know_the_canvas_cut", city_ids)
        self.assertIn("read_the_sail_list", city_ids)
        self.assertNotIn("read_the_sail_list", marsh_ids)

    def test_net_tarred_changes_hoist_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_sail")["actions"])
        tarred = replay(self.content, 1, "marsh_scout", _trace("cross_net_sail")["actions"])
        self.assertEqual(plain.state.location, "sail.hoist")
        self.assertEqual(tarred.state.location, "sail.hoist")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        tarred_ids = {a.id for a in enumerate_legal(tarred.state, self.content)}
        self.assertNotIn("tar_the_twine", plain_ids)
        self.assertIn("tar_the_twine", tarred_ids)
        self.assertIn("net_tarred", tarred.state.outcomes)

    def test_player_plain_language_hoists_sail(self) -> None:
        ...
        self.assertIn("sail_hoisted", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to sail loft"):
    ...
    def test_i4_rejects_tampered_build_on_sail_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_sail_hoisted"))
