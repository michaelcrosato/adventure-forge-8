"""Breast-hook House: score-bolt-fay outcome, sheet divergence, trestle-tree-house cross-effect."""
...
class BreastHookHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_hook_fayed_trace_replays(self) -> None:
        trace = _trace("marsh_hook_fayed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("hook_fayed", result.state.outcomes)
        self.assertEqual(result.state.location, "brh.fay")
        self.assertTrue(self.content.outcome_ready("hook_fayed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_brh")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_brh")["actions"])
        self.assertEqual(marsh.state.location, "brh.yard")
        ...
        self.assertIn("know_the_crook", marsh_ids)
        self.assertNotIn("know_the_crook", city_ids)
        self.assertIn("read_the_hook_list", city_ids)
        self.assertNotIn("read_the_hook_list", marsh_ids)

    def test_bib_seized_changes_crook_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_brh")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_trs_brh")["actions"])
        self.assertEqual(plain.state.location, "brh.crook")
        self.assertEqual(primed.state.location, "brh.crook")
        ...
        self.assertNotIn("seize_the_crook", plain_ids)
        self.assertIn("seize_the_crook", primed_ids)
        self.assertIn("bib_seized", primed.state.outcomes)

    def test_player_plain_language_fays_hook(self) -> None:
        ...
        self.assertIn("hook_fayed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the bow-hook"):
    ...
    def test_i4_rejects_tampered_build_on_brh_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_hook_fayed"))
