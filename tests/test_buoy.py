"""Buoy Yard: stuff-lash-drop outcome, sheet divergence, chart-house cross-effect."""
...
class BuoyYardTests(unittest.TestCase):
    @classmethod
    ...
    def test_buoy_set_trace_replays(self) -> None:
        trace = _trace("marsh_buoy_set")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("buoy_set", result.state.outcomes)
        self.assertEqual(result.state.location, "buoy.drop")
        self.assertTrue(self.content.outcome_ready("buoy_set", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_buoy")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_buoy")["actions"])
        self.assertEqual(marsh.state.location, "buoy.yard")
        ...
        self.assertIn("know_the_shoal_line", marsh_ids)
        self.assertNotIn("know_the_shoal_line", city_ids)
        self.assertIn("read_the_buoy_list", city_ids)
        self.assertNotIn("read_the_buoy_list", marsh_ids)

    def test_rutter_sealed_changes_drop_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_buoy")["actions"])
        placed = replay(self.content, 1, "marsh_scout", _trace("cross_chart_buoy")["actions"])
        self.assertEqual(plain.state.location, "buoy.drop")
        self.assertEqual(placed.state.location, "buoy.drop")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        placed_ids = {a.id for a in enumerate_legal(placed.state, self.content)}
        self.assertNotIn("place_by_rutter", plain_ids)
        self.assertIn("place_by_rutter", placed_ids)
        self.assertIn("rutter_sealed", placed.state.outcomes)

    def test_player_plain_language_drops_buoy(self) -> None:
        ...
        self.assertIn("buoy_set", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the buoys"):
    ...
    def test_i4_rejects_tampered_build_on_buoy_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_buoy_set"))
