"""Lodging-knee House: scribe-bolt-nick outcome, sheet divergence, breast-hook-house cross-effect."""
...
class LodgingKneeHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_lodging_nicked_trace_replays(self) -> None:
        trace = _trace("marsh_lodging_nicked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("lodging_nicked", result.state.outcomes)
        self.assertEqual(result.state.location, "lod.nick")
        self.assertTrue(self.content.outcome_ready("lodging_nicked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_lod")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_lod")["actions"])
        self.assertEqual(marsh.state.location, "lod.yard")
        ...
        self.assertIn("know_the_lodging", marsh_ids)
        self.assertNotIn("know_the_lodging", city_ids)
        self.assertIn("read_the_lodging_list", city_ids)
        self.assertNotIn("read_the_lodging_list", marsh_ids)

    def test_hook_fayed_changes_lodging_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_lod")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_brh_lod")["actions"])
        self.assertEqual(plain.state.location, "lod.lodging")
        self.assertEqual(primed.state.location, "lod.lodging")
        ...
        self.assertNotIn("fay_the_lodging", plain_ids)
        self.assertIn("fay_the_lodging", primed_ids)
        self.assertIn("hook_fayed", primed.state.outcomes)

    def test_player_plain_language_nicks_lodging(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to the lodging-knee",
            "go to lodging yard",
            "know the lodging",
            "go to the lodging",
            "scribe the lodging",
        ...
        self.assertIn("lodging_nicked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the lodging-knee"):
    ...
    def test_i4_rejects_tampered_build_on_lod_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_lodging_nicked"))
