"""Putty House: whip-knife-dust outcome, sheet divergence, sash-house cross-effect."""
...
class PuttyHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_light_dusted_trace_replays(self) -> None:
        trace = _trace("marsh_light_dusted")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("light_dusted", result.state.outcomes)
        self.assertEqual(result.state.location, "putty.dust")
        self.assertTrue(self.content.outcome_ready("light_dusted", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_putty")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_putty")["actions"])
        self.assertEqual(marsh.state.location, "putty.yard")
        ...
        self.assertIn("know_the_whip", marsh_ids)
        self.assertNotIn("know_the_whip", city_ids)
        self.assertIn("read_the_putty_list", city_ids)
        self.assertNotIn("read_the_putty_list", marsh_ids)

    def test_sash_pinned_changes_whip_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_putty")["actions"])
        sashed = replay(self.content, 1, "marsh_scout", _trace("cross_sash_putty")["actions"])
        self.assertEqual(plain.state.location, "putty.whip")
        self.assertEqual(sashed.state.location, "putty.whip")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        sashed_ids = {a.id for a in enumerate_legal(sashed.state, self.content)}
        self.assertNotIn("sash_the_bed", plain_ids)
        self.assertIn("sash_the_bed", sashed_ids)
        self.assertIn("sash_pinned", sashed.state.outcomes)

    def test_player_plain_language_dusts_light(self) -> None:
        ...
        self.assertIn("light_dusted", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the putty"):
    ...
    def test_i4_rejects_tampered_build_on_putty_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_light_dusted"))
