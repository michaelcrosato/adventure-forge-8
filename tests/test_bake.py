"""Bakehouse: set-heat-draw outcome, sheet divergence, osier-holt cross-effect."""
...
class BakehouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_loaf_drawn_trace_replays(self) -> None:
        trace = _trace("marsh_loaf_drawn")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("loaf_drawn", result.state.outcomes)
        self.assertEqual(result.state.location, "bake.hatch")
        self.assertTrue(self.content.outcome_ready("loaf_drawn", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_bake")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_bake")["actions"])
        self.assertEqual(marsh.state.location, "bake.yard")
        ...
        self.assertIn("know_the_sponge", marsh_ids)
        self.assertNotIn("know_the_sponge", city_ids)
        self.assertIn("read_the_bake_list", city_ids)
        self.assertNotIn("read_the_bake_list", marsh_ids)

    def test_frail_woven_changes_sponge_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_bake")["actions"])
        proofed = replay(self.content, 1, "marsh_scout", _trace("cross_osier_bake")["actions"])
        self.assertEqual(plain.state.location, "bake.sponge")
        self.assertEqual(proofed.state.location, "bake.sponge")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        proofed_ids = {a.id for a in enumerate_legal(proofed.state, self.content)}
        self.assertNotIn("proof_in_frail", plain_ids)
        self.assertIn("proof_in_frail", proofed_ids)
        self.assertIn("frail_woven", proofed.state.outcomes)

    def test_player_plain_language_draws_loaf(self) -> None:
        ...
        self.assertIn("loaf_drawn", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the bakehouse"):
    ...
    def test_i4_rejects_tampered_build_on_bake_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_loaf_drawn"))
