"""Bindery: gather-sew-nip outcome, sheet divergence, glue-house cross-effect."""
...
class BinderyTests(unittest.TestCase):
    @classmethod
    ...
    def test_book_bound_trace_replays(self) -> None:
        trace = _trace("marsh_book_bound")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("book_bound", result.state.outcomes)
        self.assertEqual(result.state.location, "bind.nip")
        self.assertTrue(self.content.outcome_ready("book_bound", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_bind")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_bind")["actions"])
        self.assertEqual(marsh.state.location, "bind.yard")
        ...
        self.assertIn("know_the_quire", marsh_ids)
        self.assertNotIn("know_the_quire", city_ids)
        self.assertIn("read_the_bind_list", city_ids)
        self.assertNotIn("read_the_bind_list", marsh_ids)

    def test_glue_caked_changes_quire_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_bind")["actions"])
        glued = replay(self.content, 1, "marsh_scout", _trace("cross_glue_bind")["actions"])
        self.assertEqual(plain.state.location, "bind.quire")
        self.assertEqual(glued.state.location, "bind.quire")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        glued_ids = {a.id for a in enumerate_legal(glued.state, self.content)}
        self.assertNotIn("glue_the_quire", plain_ids)
        self.assertIn("glue_the_quire", glued_ids)
        self.assertIn("glue_caked", glued.state.outcomes)

    def test_player_plain_language_binds_book(self) -> None:
        ...
        self.assertIn("book_bound", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the bindery"):
    ...
    def test_i4_rejects_tampered_build_on_bind_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_book_bound"))
