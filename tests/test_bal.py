"""Baluster House: rip-flute-shoulder outcome, sheet divergence, handrail-house cross-effect."""
...
class BalusterHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_neck_shouldered_trace_replays(self) -> None:
        trace = _trace("marsh_neck_shouldered")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("neck_shouldered", result.state.outcomes)
        self.assertEqual(result.state.location, "bal.neck")
        self.assertTrue(self.content.outcome_ready("neck_shouldered", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_bal")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_bal")["actions"])
        self.assertEqual(marsh.state.location, "bal.yard")
        ...
        self.assertIn("know_the_square", marsh_ids)
        self.assertNotIn("know_the_square", city_ids)
        self.assertIn("read_the_baluster_list", city_ids)
        self.assertNotIn("read_the_baluster_list", marsh_ids)

    def test_ramp_wreathed_changes_square_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_bal")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_hand_bal")["actions"])
        self.assertEqual(plain.state.location, "bal.square")
        self.assertEqual(primed.state.location, "bal.square")
        ...
        self.assertNotIn("wreath_the_square", plain_ids)
        self.assertIn("wreath_the_square", primed_ids)
        self.assertIn("ramp_wreathed", primed.state.outcomes)

    def test_player_plain_language_shoulders_neck(self) -> None:
        ...
        self.assertIn("neck_shouldered", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the balusters"):
    ...
    def test_i4_rejects_tampered_build_on_baluster_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_neck_shouldered"))
