"""Oyster Park: spat-seed-cull outcome, sheet divergence, windpump cross-effect."""
...
class OysterParkTests(unittest.TestCase):
    @classmethod
    ...
    def test_oyster_culled_trace_replays(self) -> None:
        trace = _trace("marsh_oyster_culled")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("oyster_culled", result.state.outcomes)
        self.assertEqual(result.state.location, "oyster.shed")
        self.assertTrue(self.content.outcome_ready("oyster_culled", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_oyster")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_oyster")["actions"])
        self.assertEqual(marsh.state.location, "oyster.yard")
        ...
        self.assertIn("know_the_spat_set", marsh_ids)
        self.assertNotIn("know_the_spat_set", city_ids)
        self.assertIn("read_the_bed_list", city_ids)
        self.assertNotIn("read_the_bed_list", marsh_ids)

    def test_flats_drained_changes_bed_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_oyster")["actions"])
        drained = replay(self.content, 1, "marsh_scout", _trace("cross_pump_oyster")["actions"])
        self.assertEqual(plain.state.location, "oyster.beds")
        self.assertEqual(drained.state.location, "oyster.beds")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        drained_ids = {a.id for a in enumerate_legal(drained.state, self.content)}
        self.assertNotIn("work_the_dry_beds", plain_ids)
        self.assertIn("work_the_dry_beds", drained_ids)
        self.assertIn("flats_drained", drained.state.outcomes)

    def test_player_plain_language_culls_oysters(self) -> None:
        ...
        lines = [
            ...
            "go to oyster yard",
            "know the spat set",
            "go to the spat house",
            "take spat bag",
            "go to oyster yard",
            "go to the oyster beds",
            "seed the beds",
            "go to oyster yard",
            "go to the cull shed",
            "cull the beds",
        ...
        self.assertIn("oyster_culled", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to oyster park"):
    ...
    def test_i4_rejects_tampered_build_on_oyster_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_oyster_culled"))
