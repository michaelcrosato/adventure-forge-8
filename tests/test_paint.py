"""Paint House: mull-mix-brush outcome, sheet divergence, putty-house cross-effect."""
...
class PaintHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_coat_brushed_trace_replays(self) -> None:
        trace = _trace("marsh_coat_brushed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("coat_brushed", result.state.outcomes)
        self.assertEqual(result.state.location, "paint.brush")
        self.assertTrue(self.content.outcome_ready("coat_brushed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_paint")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_paint")["actions"])
        self.assertEqual(marsh.state.location, "paint.yard")
        ...
        self.assertIn("know_the_mull", marsh_ids)
        self.assertNotIn("know_the_mull", city_ids)
        self.assertIn("read_the_paint_list", city_ids)
        self.assertNotIn("read_the_paint_list", marsh_ids)

    def test_light_dusted_changes_mull_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_paint")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_putty_paint")["actions"])
        self.assertEqual(plain.state.location, "paint.mull")
        self.assertEqual(primed.state.location, "paint.mull")
        ...
        self.assertNotIn("dust_the_coat", plain_ids)
        self.assertIn("dust_the_coat", primed_ids)
        self.assertIn("light_dusted", primed.state.outcomes)

    def test_player_plain_language_brushes_coat(self) -> None:
        ...
        self.assertIn("coat_brushed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the paint"):
    ...
    def test_i4_rejects_tampered_build_on_paint_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_coat_brushed"))
