"""Fid House: turn-ream-seize outcome, sheet divergence, stem-house cross-effect."""
...
class FidHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_eye_seized_trace_replays(self) -> None:
        trace = _trace("marsh_eye_seized")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("eye_seized", result.state.outcomes)
        self.assertEqual(result.state.location, "fid.splice")
        self.assertTrue(self.content.outcome_ready("eye_seized", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_fid")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_fid")["actions"])
        self.assertEqual(marsh.state.location, "fid.yard")
        ...
        self.assertIn("know_the_turn", marsh_ids)
        self.assertNotIn("know_the_turn", city_ids)
        self.assertIn("read_the_fid_list", city_ids)
        self.assertNotIn("read_the_fid_list", marsh_ids)

    def test_knee_hung_changes_turn_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_fid")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_stem_fid")["actions"])
        self.assertEqual(plain.state.location, "fid.turn")
        self.assertEqual(primed.state.location, "fid.turn")
        ...
        self.assertNotIn("hang_the_fid", plain_ids)
        self.assertIn("hang_the_fid", primed_ids)
        self.assertIn("knee_hung", primed.state.outcomes)

    def test_player_plain_language_seizes_eye(self) -> None:
        ...
        self.assertIn("eye_seized", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the fid"):
    ...
    def test_i4_rejects_tampered_build_on_fid_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_eye_seized"))
