"""Mould Loft: loft-spile-bevel outcome, sheet divergence, windlass cross-effect."""
...
class MouldLoftTests(unittest.TestCase):
    @classmethod
    ...
    def test_station_bevelled_trace_replays(self) -> None:
        trace = _trace("marsh_station_bevelled")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("station_bevelled", result.state.outcomes)
        self.assertEqual(result.state.location, "offs.bevel")
        self.assertTrue(self.content.outcome_ready("station_bevelled", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_offs")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_offs")["actions"])
        self.assertEqual(marsh.state.location, "offs.yard")
        ...
        self.assertIn("know_the_grid", marsh_ids)
        self.assertNotIn("know_the_grid", city_ids)
        self.assertIn("read_the_offset_list", city_ids)
        self.assertNotIn("read_the_offset_list", marsh_ids)

    def test_round_heaved_changes_grid_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_offs")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_wind_offs")["actions"])
        self.assertEqual(plain.state.location, "offs.grid")
        self.assertEqual(primed.state.location, "offs.grid")
        ...
        self.assertNotIn("heave_the_grid", plain_ids)
        self.assertIn("heave_the_grid", primed_ids)
        self.assertIn("round_heaved", primed.state.outcomes)

    def test_player_plain_language_bevels_station(self) -> None:
        ...
        self.assertIn("station_bevelled", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the offsets"):
    ...
    def test_i4_rejects_tampered_build_on_offs_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_station_bevelled"))
