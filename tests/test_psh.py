"""Planksheer House: score-plane-dump outcome, sheet divergence, tabernacle-house cross-effect."""
...
class PlanksheerHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_edge_dumped_trace_replays(self) -> None:
        trace = _trace("marsh_edge_dumped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("edge_dumped", result.state.outcomes)
        self.assertEqual(result.state.location, "psh.edge")
        self.assertTrue(self.content.outcome_ready("edge_dumped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_psh")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_psh")["actions"])
        self.assertEqual(marsh.state.location, "psh.yard")
        ...
        self.assertIn("know_the_sheer", marsh_ids)
        self.assertNotIn("know_the_sheer", city_ids)
        self.assertIn("read_the_planksheer_list", city_ids)
        self.assertNotIn("read_the_planksheer_list", marsh_ids)

    def test_keeper_pinned_changes_sheer_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_psh")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_tab_psh")["actions"])
        self.assertEqual(plain.state.location, "psh.sheer")
        self.assertEqual(primed.state.location, "psh.sheer")
        ...
        self.assertNotIn("pin_the_sheer", plain_ids)
        self.assertIn("pin_the_sheer", primed_ids)
        self.assertIn("keeper_pinned", primed.state.outcomes)

    def test_player_plain_language_dumps_edge(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to the planksheer",
            "go to planksheer yard",
            "know the sheer",
            "go to the sheer",
            "score the sheer",
            "go to planksheer yard",
        ...
        self.assertIn("edge_dumped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the planksheer"):
    ...
    def test_i4_rejects_tampered_build_on_psh_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_edge_dumped"))
