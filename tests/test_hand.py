"""Handrail House: stick-scarf-wreath outcome, sheet divergence, newel-house cross-effect."""
...
class HandrailHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_ramp_wreathed_trace_replays(self) -> None:
        trace = _trace("marsh_ramp_wreathed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("ramp_wreathed", result.state.outcomes)
        self.assertEqual(result.state.location, "hand.wreath")
        self.assertTrue(self.content.outcome_ready("ramp_wreathed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_hand")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_hand")["actions"])
        self.assertEqual(marsh.state.location, "hand.yard")
        ...
        self.assertIn("know_the_stick", marsh_ids)
        self.assertNotIn("know_the_stick", city_ids)
        self.assertIn("read_the_handrail_list", city_ids)
        self.assertNotIn("read_the_handrail_list", marsh_ids)

    def test_finial_dowelled_changes_stick_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_hand")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_newel_hand")["actions"])
        self.assertEqual(plain.state.location, "hand.stick")
        self.assertEqual(primed.state.location, "hand.stick")
        ...
        self.assertNotIn("newel_the_stick", plain_ids)
        self.assertIn("newel_the_stick", primed_ids)
        self.assertIn("finial_dowelled", primed.state.outcomes)

    def test_player_plain_language_wreaths_ramp(self) -> None:
        ...
        self.assertIn("ramp_wreathed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the handrail"):
    ...
    def test_i4_rejects_tampered_build_on_handrail_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_ramp_wreathed"))
