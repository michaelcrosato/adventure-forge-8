"""Mustard Mill: mill-wet-pot outcome, sheet divergence, cider-house cross-effect."""
...
class MustardMillTests(unittest.TestCase):
    @classmethod
    ...
    def test_mustard_potted_trace_replays(self) -> None:
        trace = _trace("marsh_mustard_potted")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("mustard_potted", result.state.outcomes)
        self.assertEqual(result.state.location, "must.pot")
        self.assertTrue(self.content.outcome_ready("mustard_potted", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_must")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_must")["actions"])
        self.assertEqual(marsh.state.location, "must.yard")
        ...
        self.assertIn("know_the_seed", marsh_ids)
        self.assertNotIn("know_the_seed", city_ids)
        self.assertIn("read_the_mustard_list", city_ids)
        self.assertNotIn("read_the_mustard_list", marsh_ids)

    def test_keeve_bunged_changes_quern_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_must")["actions"])
        wet = replay(self.content, 1, "marsh_scout", _trace("cross_cider_must")["actions"])
        self.assertEqual(plain.state.location, "must.quern")
        self.assertEqual(wet.state.location, "must.quern")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        wet_ids = {a.id for a in enumerate_legal(wet.state, self.content)}
        self.assertNotIn("cider_the_quern", plain_ids)
        self.assertIn("cider_the_quern", wet_ids)
        self.assertIn("keeve_bunged", wet.state.outcomes)

    def test_player_plain_language_pots_mustard(self) -> None:
        ...
        self.assertIn("mustard_potted", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the mustard"):
    ...
    def test_i4_rejects_tampered_build_on_must_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_mustard_potted"))
