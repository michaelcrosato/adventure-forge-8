"""Cathead House: adze-bush-fish outcome, sheet divergence, tiller-house cross-effect."""
...
class CatheadHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_fluke_fished_trace_replays(self) -> None:
        trace = _trace("marsh_fluke_fished")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("fluke_fished", result.state.outcomes)
        self.assertEqual(result.state.location, "cath.fluke")
        self.assertTrue(self.content.outcome_ready("fluke_fished", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_cath")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_cath")["actions"])
        self.assertEqual(marsh.state.location, "cath.yard")
        ...
        self.assertIn("know_the_bill", marsh_ids)
        self.assertNotIn("know_the_bill", city_ids)
        self.assertIn("read_the_cathead_list", city_ids)
        self.assertNotIn("read_the_cathead_list", marsh_ids)

    def test_tiller_yoked_changes_bill_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_cath")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_till_cath")["actions"])
        self.assertEqual(plain.state.location, "cath.bill")
        self.assertEqual(primed.state.location, "cath.bill")
        ...
        self.assertNotIn("yoke_the_bill", plain_ids)
        self.assertIn("yoke_the_bill", primed_ids)
        self.assertIn("tiller_yoked", primed.state.outcomes)

    def test_player_plain_language_fishes_fluke(self) -> None:
        ...
        self.assertIn("fluke_fished", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the cathead"):
    ...
    def test_i4_rejects_tampered_build_on_cath_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_fluke_fished"))
