"""Skirting House: cope-plane-fix outcome, sheet divergence, casing-house cross-effect."""
...
class SkirtingHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_plinth_fixed_trace_replays(self) -> None:
        trace = _trace("marsh_plinth_fixed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("plinth_fixed", result.state.outcomes)
        self.assertEqual(result.state.location, "skirt.plinth")
        self.assertTrue(self.content.outcome_ready("plinth_fixed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_skirt")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_skirt")["actions"])
        self.assertEqual(marsh.state.location, "skirt.yard")
        ...
        self.assertIn("know_the_cope", marsh_ids)
        self.assertNotIn("know_the_cope", city_ids)
        self.assertIn("read_the_skirting_list", city_ids)
        self.assertNotIn("read_the_skirting_list", marsh_ids)

    def test_casing_tacked_changes_cope_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_skirt")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_case_skirt")["actions"])
        self.assertEqual(plain.state.location, "skirt.cope")
        self.assertEqual(primed.state.location, "skirt.cope")
        ...
        self.assertNotIn("case_the_cope", plain_ids)
        self.assertIn("case_the_cope", primed_ids)
        self.assertIn("casing_tacked", primed.state.outcomes)

    def test_player_plain_language_fixes_plinth(self) -> None:
        ...
        self.assertIn("plinth_fixed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the skirting"):
    ...
    def test_i4_rejects_tampered_build_on_skirting_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_plinth_fixed"))
