"""Wheelwright: dish-spoke-tyre outcome, sheet divergence, nailery cross-effect."""
...
class WheelwrightTests(unittest.TestCase):
    @classmethod
    ...
    def test_tyre_set_trace_replays(self) -> None:
        trace = _trace("marsh_tyre_set")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("tyre_set", result.state.outcomes)
        self.assertEqual(result.state.location, "wain.felloe")
        self.assertTrue(self.content.outcome_ready("tyre_set", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_wain")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_wain")["actions"])
        self.assertEqual(marsh.state.location, "wain.yard")
        ...
        self.assertIn("know_the_hub", marsh_ids)
        self.assertNotIn("know_the_hub", city_ids)
        self.assertIn("read_the_wain_list", city_ids)
        self.assertNotIn("read_the_wain_list", marsh_ids)

    def test_nail_pointed_changes_hub_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_wain")["actions"])
        boxed = replay(self.content, 1, "marsh_scout", _trace("cross_nail_wain")["actions"])
        self.assertEqual(plain.state.location, "wain.hub")
        self.assertEqual(boxed.state.location, "wain.hub")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        boxed_ids = {a.id for a in enumerate_legal(boxed.state, self.content)}
        self.assertNotIn("nail_the_box", plain_ids)
        self.assertIn("nail_the_box", boxed_ids)
        self.assertIn("nail_pointed", boxed.state.outcomes)

    def test_player_plain_language_sets_tyre(self) -> None:
        ...
        self.assertIn("tyre_set", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the wains"):
    ...
    def test_i4_rejects_tampered_build_on_wain_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_tyre_set"))
