"""Grate House: swage-rivet-register outcome, sheet divergence, fireback cross-effect."""
...
class GrateHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_slide_registered_trace_replays(self) -> None:
        trace = _trace("marsh_slide_registered")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("slide_registered", result.state.outcomes)
        self.assertEqual(result.state.location, "grate.slide")
        self.assertTrue(self.content.outcome_ready("slide_registered", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_grate")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_grate")["actions"])
        self.assertEqual(marsh.state.location, "grate.yard")
        ...
        self.assertIn("know_the_swage", marsh_ids)
        self.assertNotIn("know_the_swage", city_ids)
        self.assertIn("read_the_grate_list", city_ids)
        self.assertNotIn("read_the_grate_list", marsh_ids)

    def test_back_bedded_changes_swage_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_grate")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_fb_grate")["actions"])
        self.assertEqual(plain.state.location, "grate.swage")
        self.assertEqual(primed.state.location, "grate.swage")
        ...
        self.assertNotIn("bed_the_swage", plain_ids)
        self.assertIn("bed_the_swage", primed_ids)
        self.assertIn("back_bedded", primed.state.outcomes)

    def test_player_plain_language_registers_slide(self) -> None:
        ...
        self.assertIn("slide_registered", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the grate"):
    ...
    def test_i4_rejects_tampered_build_on_grate_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_slide_registered"))
