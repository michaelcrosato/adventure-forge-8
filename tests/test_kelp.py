"""Kelp Shore: cut-dry-burn outcome, sheet divergence, buoy-yard cross-effect."""
...
class KelpShoreTests(unittest.TestCase):
    @classmethod
    ...
    def test_kelp_burned_trace_replays(self) -> None:
        trace = _trace("marsh_kelp_burned")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("kelp_burned", result.state.outcomes)
        self.assertEqual(result.state.location, "kelp.hearth")
        self.assertTrue(self.content.outcome_ready("kelp_burned", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_kelp")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_kelp")["actions"])
        self.assertEqual(marsh.state.location, "kelp.yard")
        ...
        self.assertIn("know_the_wrack_tide", marsh_ids)
        self.assertNotIn("know_the_wrack_tide", city_ids)
        self.assertIn("read_the_kelp_list", city_ids)
        self.assertNotIn("read_the_kelp_list", marsh_ids)

    def test_buoy_set_changes_bank_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_kelp")["actions"])
        outer = replay(self.content, 1, "marsh_scout", _trace("cross_buoy_kelp")["actions"])
        self.assertEqual(plain.state.location, "kelp.bank")
        self.assertEqual(outer.state.location, "kelp.bank")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        outer_ids = {a.id for a in enumerate_legal(outer.state, self.content)}
        self.assertNotIn("cut_the_outer_bank", plain_ids)
        self.assertIn("cut_the_outer_bank", outer_ids)
        self.assertIn("buoy_set", outer.state.outcomes)

    def test_player_plain_language_burns_kelp(self) -> None:
        ...
        self.assertIn("kelp_burned", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the kelp"):
    ...
    def test_i4_rejects_tampered_build_on_kelp_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_kelp_burned"))
