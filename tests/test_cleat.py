"""Cleat House: saw-gouge-bolt outcome, sheet divergence, fid-house cross-effect."""
...
class CleatHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_base_bolted_trace_replays(self) -> None:
        trace = _trace("marsh_base_bolted")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("base_bolted", result.state.outcomes)
        self.assertEqual(result.state.location, "cleat.bolt")
        self.assertTrue(self.content.outcome_ready("base_bolted", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_cleat")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_cleat")["actions"])
        self.assertEqual(marsh.state.location, "cleat.yard")
        ...
        self.assertIn("know_the_saw", marsh_ids)
        self.assertNotIn("know_the_saw", city_ids)
        self.assertIn("read_the_cleat_list", city_ids)
        self.assertNotIn("read_the_cleat_list", marsh_ids)

    def test_eye_seized_changes_saw_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_cleat")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_fid_cleat")["actions"])
        self.assertEqual(plain.state.location, "cleat.saw")
        self.assertEqual(primed.state.location, "cleat.saw")
        ...
        self.assertNotIn("seize_the_saw", plain_ids)
        self.assertIn("seize_the_saw", primed_ids)
        self.assertIn("eye_seized", primed.state.outcomes)

    def test_player_plain_language_bolts_base(self) -> None:
        ...
        self.assertIn("base_bolted", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the cleat"):
    ...
    def test_i4_rejects_tampered_build_on_cleat_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_base_bolted"))
