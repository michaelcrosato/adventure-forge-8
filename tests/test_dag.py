"""Dagger-knee House: fair-side-mark outcome, sheet divergence, lodging-knee-house cross-effect."""
...
class DaggerKneeHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_sirmark_marked_trace_replays(self) -> None:
        trace = _trace("marsh_sirmark_marked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("sirmark_marked", result.state.outcomes)
        self.assertEqual(result.state.location, "dag.sirmark")
        self.assertTrue(self.content.outcome_ready("sirmark_marked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_dag")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_dag")["actions"])
        self.assertEqual(marsh.state.location, "dag.yard")
        ...
        self.assertIn("know_the_dagger", marsh_ids)
        self.assertNotIn("know_the_dagger", city_ids)
        self.assertIn("read_the_dagger_list", city_ids)
        self.assertNotIn("read_the_dagger_list", marsh_ids)

    def test_lodging_nicked_changes_dagger_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_dag")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_lod_dag")["actions"])
        self.assertEqual(plain.state.location, "dag.dagger")
        self.assertEqual(primed.state.location, "dag.dagger")
        ...
        self.assertNotIn("nick_the_dagger", plain_ids)
        self.assertIn("nick_the_dagger", primed_ids)
        self.assertIn("lodging_nicked", primed.state.outcomes)

    def test_player_plain_language_marks_sirmark(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to the dagger-knee",
            "go to dagger yard",
            "know the dagger",
            "go to the dagger",
            "fair the dagger",
        ...
        self.assertIn("sirmark_marked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the dagger-knee"):
    ...
    def test_i4_rejects_tampered_build_on_dag_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_sirmark_marked"))
