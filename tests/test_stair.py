"""Stair House: gauge-saw-wedge outcome, sheet divergence, cornice-house cross-effect."""
...
class StairHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_riser_wedged_trace_replays(self) -> None:
        trace = _trace("marsh_riser_wedged")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("riser_wedged", result.state.outcomes)
        self.assertEqual(result.state.location, "stair.riser")
        self.assertTrue(self.content.outcome_ready("riser_wedged", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_stair")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_stair")["actions"])
        self.assertEqual(marsh.state.location, "stair.yard")
        ...
        self.assertIn("know_the_string", marsh_ids)
        self.assertNotIn("know_the_string", city_ids)
        self.assertIn("read_the_stair_list", city_ids)
        self.assertNotIn("read_the_stair_list", marsh_ids)

    def test_cornice_floated_changes_string_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_stair")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_corn_stair")["actions"])
        self.assertEqual(plain.state.location, "stair.string")
        self.assertEqual(primed.state.location, "stair.string")
        ...
        self.assertNotIn("float_the_string", plain_ids)
        self.assertIn("float_the_string", primed_ids)
        self.assertIn("cornice_floated", primed.state.outcomes)

    def test_player_plain_language_wedges_riser(self) -> None:
        ...
        self.assertIn("riser_wedged", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the stairs"):
    ...
    def test_i4_rejects_tampered_build_on_stair_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_riser_wedged"))
