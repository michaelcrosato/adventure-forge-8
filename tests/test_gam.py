"""Gammoning House: woold-ride-frap outcome, sheet divergence, bitts-house cross-effect."""
...
class GammoningHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_gammon_frapped_trace_replays(self) -> None:
        trace = _trace("marsh_gammon_frapped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("gammon_frapped", result.state.outcomes)
        self.assertEqual(result.state.location, "gam.frap")
        self.assertTrue(self.content.outcome_ready("gammon_frapped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_gam")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_gam")["actions"])
        self.assertEqual(marsh.state.location, "gam.yard")
        ...
        self.assertIn("know_the_woold", marsh_ids)
        self.assertNotIn("know_the_woold", city_ids)
        self.assertIn("read_the_gammon_list", city_ids)
        self.assertNotIn("read_the_gammon_list", marsh_ids)

    def test_cable_belayed_changes_woold_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_gam")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_bitt_gam")["actions"])
        self.assertEqual(plain.state.location, "gam.woold")
        self.assertEqual(primed.state.location, "gam.woold")
        ...
        self.assertNotIn("belay_the_woold", plain_ids)
        self.assertIn("belay_the_woold", primed_ids)
        self.assertIn("cable_belayed", primed.state.outcomes)

    def test_player_plain_language_fraps_gammon(self) -> None:
        ...
        self.assertIn("gammon_frapped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the gammon"):
    ...
    def test_i4_rejects_tampered_build_on_gam_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_gammon_frapped"))
