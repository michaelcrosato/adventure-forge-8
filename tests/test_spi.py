"""Spirketting House: score-plane-clench outcome, sheet divergence, planksheer-house cross-effect."""
...
class SpirkettingHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_spirket_clenched_trace_replays(self) -> None:
        trace = _trace("marsh_spirket_clenched")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("spirket_clenched", result.state.outcomes)
        self.assertEqual(result.state.location, "spi.ring")
        self.assertTrue(self.content.outcome_ready("spirket_clenched", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_spi")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_spi")["actions"])
        self.assertEqual(marsh.state.location, "spi.yard")
        ...
        self.assertIn("know_the_spirket", marsh_ids)
        self.assertNotIn("know_the_spirket", city_ids)
        self.assertIn("read_the_spirketting_list", city_ids)
        self.assertNotIn("read_the_spirketting_list", marsh_ids)

    def test_edge_dumped_changes_spirket_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_spi")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_psh_spi")["actions"])
        self.assertEqual(plain.state.location, "spi.spirket")
        self.assertEqual(primed.state.location, "spi.spirket")
        ...
        self.assertNotIn("dump_the_spirket", plain_ids)
        self.assertIn("dump_the_spirket", primed_ids)
        self.assertIn("edge_dumped", primed.state.outcomes)

    def test_player_plain_language_clenches_spirket(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to the spirketting",
            "go to spirketting yard",
            "know the spirket",
            "go to the spirket",
            "score the spirket",
            "go to spirketting yard",
        ...
        self.assertIn("spirket_clenched", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the spirketting"):
    ...
    def test_i4_rejects_tampered_build_on_spi_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_spirket_clenched"))
