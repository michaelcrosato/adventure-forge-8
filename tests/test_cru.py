"""Crutch House: adze-fit-spike outcome, sheet divergence, keelson-house cross-effect."""
...
class CrutchHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_crotch_spiked_trace_replays(self) -> None:
        trace = _trace("marsh_crotch_spiked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("crotch_spiked", result.state.outcomes)
        self.assertEqual(result.state.location, "cru.crotch")
        self.assertTrue(self.content.outcome_ready("crotch_spiked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_cru")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_cru")["actions"])
        self.assertEqual(marsh.state.location, "cru.yard")
        ...
        self.assertIn("know_the_crutch", marsh_ids)
        self.assertNotIn("know_the_crutch", city_ids)
        self.assertIn("read_the_crutch_list", city_ids)
        self.assertNotIn("read_the_crutch_list", marsh_ids)

    def test_sister_bolted_changes_crutch_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_cru")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_ksn_cru")["actions"])
        self.assertEqual(plain.state.location, "cru.adze")
        self.assertEqual(primed.state.location, "cru.adze")
        ...
        self.assertNotIn("bolt_the_crutch", plain_ids)
        self.assertIn("bolt_the_crutch", primed_ids)
        self.assertIn("sister_bolted", primed.state.outcomes)

    def test_player_plain_language_spikes_crotch(self) -> None:
        ...
        self.assertIn("crotch_spiked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the crutch"):
    ...
    def test_i4_rejects_tampered_build_on_cru_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_crotch_spiked"))
