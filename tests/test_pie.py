"""Pie House: roll-fill-crimp outcome, sheet divergence, sausage-house cross-effect."""
...
class PieHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_pie_crimped_trace_replays(self) -> None:
        trace = _trace("marsh_pie_crimped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("pie_crimped", result.state.outcomes)
        self.assertEqual(result.state.location, "pie.crimp")
        self.assertTrue(self.content.outcome_ready("pie_crimped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_pie")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_pie")["actions"])
        self.assertEqual(marsh.state.location, "pie.yard")
        ...
        self.assertIn("know_the_crust", marsh_ids)
        self.assertNotIn("know_the_crust", city_ids)
        self.assertIn("read_the_pie_list", city_ids)
        self.assertNotIn("read_the_pie_list", marsh_ids)

    def test_sausage_linked_changes_crust_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_pie")["actions"])
        larded = replay(self.content, 1, "marsh_scout", _trace("cross_link_pie")["actions"])
        self.assertEqual(plain.state.location, "pie.crust")
        self.assertEqual(larded.state.location, "pie.crust")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        larded_ids = {a.id for a in enumerate_legal(larded.state, self.content)}
        self.assertNotIn("lard_the_crust", plain_ids)
        self.assertIn("lard_the_crust", larded_ids)
        self.assertIn("sausage_linked", larded.state.outcomes)

    def test_player_plain_language_crimps_pie(self) -> None:
        ...
        self.assertIn("pie_crimped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the pies"):
    ...
    def test_i4_rejects_tampered_build_on_pie_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_pie_crimped"))
