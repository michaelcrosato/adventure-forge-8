"""Pillar House: hew-square-shore outcome, sheet divergence, stanchion-house cross-effect."""
...
class PillarHouseTests(unittest.TestCase):
    ...
    def test_foot_spiked_changes_pillar_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_pil")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_stn_pil")["actions"])
        self.assertEqual(plain.state.location, "pil.samson")
        self.assertEqual(primed.state.location, "pil.samson")
        ...
        self.assertNotIn("spike_the_pillar", plain_ids)
        self.assertIn("spike_the_pillar", primed_ids)
        self.assertIn("foot_spiked", primed.state.outcomes)
