"""Deadwood Yard: dub-rabbet-bolt outcome, sheet divergence, treenail cross-effect."""
...
class DeadwoodYardTests(unittest.TestCase):
    @classmethod
    ...
    def test_hog_bolted_trace_replays(self) -> None:
        trace = _trace("marsh_hog_bolted")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("hog_bolted", result.state.outcomes)
        self.assertEqual(result.state.location, "dead.hog")
        self.assertTrue(self.content.outcome_ready("hog_bolted", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_dead")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_dead")["actions"])
        self.assertEqual(marsh.state.location, "dead.yard")
        ...
        self.assertIn("know_the_dub", marsh_ids)
        self.assertNotIn("know_the_dub", city_ids)
        self.assertIn("read_the_deadwood_list", city_ids)
        self.assertNotIn("read_the_deadwood_list", marsh_ids)

    def test_trunnel_driven_changes_dub_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_dead")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_trun_dead")["actions"])
        self.assertEqual(plain.state.location, "dead.dub")
        self.assertEqual(primed.state.location, "dead.dub")
        ...
        self.assertNotIn("drive_the_dub", plain_ids)
        self.assertIn("drive_the_dub", primed_ids)
        self.assertIn("trunnel_driven", primed.state.outcomes)

    def test_player_plain_language_bolts_hog(self) -> None:
        ...
        self.assertIn("hog_bolted", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the deadwood"):
    ...
    def test_i4_rejects_tampered_build_on_dead_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_hog_bolted"))
