"""Waterway House: snipe-dowel-pay outcome, sheet divergence, crosstree-house cross-effect."""
...
class WaterwayHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_waterway_paid_trace_replays(self) -> None:
        trace = _trace("marsh_waterway_paid")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("waterway_paid", result.state.outcomes)
        self.assertEqual(result.state.location, "wtr.pay")
        self.assertTrue(self.content.outcome_ready("waterway_paid", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_wtr")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_wtr")["actions"])
        self.assertEqual(marsh.state.location, "wtr.yard")
        ...
        self.assertIn("know_the_snipe", marsh_ids)
        self.assertNotIn("know_the_snipe", city_ids)
        self.assertIn("read_the_waterway_list", city_ids)
        self.assertNotIn("read_the_waterway_list", marsh_ids)

    def test_bolster_seized_changes_snipe_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_wtr")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_cros_wtr")["actions"])
        self.assertEqual(plain.state.location, "wtr.snipe")
        self.assertEqual(primed.state.location, "wtr.snipe")
        ...
        self.assertNotIn("seize_the_snipe", plain_ids)
        self.assertIn("seize_the_snipe", primed_ids)
        self.assertIn("bolster_seized", primed.state.outcomes)

    def test_player_plain_language_pays_waterway(self) -> None:
        ...
        self.assertIn("waterway_paid", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the waterway"):
    ...
    def test_i4_rejects_tampered_build_on_wtr_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_waterway_paid"))
