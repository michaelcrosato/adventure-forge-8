"""Bee Skeps: smoke-and-set outcome, sheet divergence, wreck-chapel cross-effect."""
...
class BeeSkepsTests(unittest.TestCase):
    @classmethod
    ...
    def test_hive_kept_trace_replays(self) -> None:
        trace = _trace("marsh_hive_kept")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("hive_kept", result.state.outcomes)
        self.assertEqual(result.state.location, "hive.shed")
        self.assertTrue(self.content.outcome_ready("hive_kept", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_hive")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_hive")["actions"])
        self.assertEqual(marsh.state.location, "hive.yard")
        ...
        self.assertIn("know_the_hive_hum", marsh_ids)
        self.assertNotIn("know_the_hive_hum", city_ids)
        self.assertIn("read_the_skep_mark", city_ids)
        self.assertNotIn("read_the_skep_mark", marsh_ids)

    def test_wreck_laid_changes_skep_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_hive")["actions"])
        wrecked = replay(self.content, 1, "marsh_scout", _trace("cross_wreck_hive")["actions"])
        self.assertEqual(plain.state.location, "hive.skeps")
        self.assertEqual(wrecked.state.location, "hive.skeps")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        wrecked_ids = {a.id for a in enumerate_legal(wrecked.state, self.content)}
        self.assertNotIn("bless_the_skep", plain_ids)
        self.assertIn("bless_the_skep", wrecked_ids)
        self.assertIn("wreck_laid", wrecked.state.outcomes)

    def test_player_plain_language_keeps_hive(self) -> None:
        ...
        self.assertIn("hive_kept", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to bee skeps"):
    ...
    def test_i4_rejects_tampered_build_on_hive_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_hive_kept"))
