"""Tabernacle House: score-mortise-pin outcome, sheet divergence, carling-house cross-effect."""
...
class TabernacleHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_keeper_pinned_trace_replays(self) -> None:
        trace = _trace("marsh_keeper_pinned")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("keeper_pinned", result.state.outcomes)
        self.assertEqual(result.state.location, "tab.keeper")
        self.assertTrue(self.content.outcome_ready("keeper_pinned", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_tab")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_tab")["actions"])
        self.assertEqual(marsh.state.location, "tab.yard")
        ...
        self.assertIn("know_the_tabernacle", marsh_ids)
        self.assertNotIn("know_the_tabernacle", city_ids)
        self.assertIn("read_the_tabernacle_list", city_ids)
        self.assertNotIn("read_the_tabernacle_list", marsh_ids)

    def test_carling_spiked_changes_socket_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_tab")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_carl_tab")["actions"])
        self.assertEqual(plain.state.location, "tab.socket")
        self.assertEqual(primed.state.location, "tab.socket")
        ...
        self.assertNotIn("spike_the_socket", plain_ids)
        self.assertIn("spike_the_socket", primed_ids)
        self.assertIn("carling_spiked", primed.state.outcomes)

    def test_player_plain_language_pins_keeper(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to the tabernacle",
            "go to tabernacle yard",
            "know the tabernacle",
            "go to the socket",
        ...
        self.assertIn("keeper_pinned", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the tabernacle"):
    ...
    def test_i4_rejects_tampered_build_on_tab_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_keeper_pinned"))
