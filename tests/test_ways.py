"""Launch Ways: grease-poppet-trip outcome, sheet divergence, oar-loft cross-effect."""
...
class LaunchWaysTests(unittest.TestCase):
    @classmethod
    ...
    def test_hull_launched_trace_replays(self) -> None:
        trace = _trace("marsh_hull_launched")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("hull_launched", result.state.outcomes)
        self.assertEqual(result.state.location, "ways.trigger")
        self.assertTrue(self.content.outcome_ready("hull_launched", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_ways")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_ways")["actions"])
        self.assertEqual(marsh.state.location, "ways.yard")
        ...
        self.assertIn("know_the_grease", marsh_ids)
        self.assertNotIn("know_the_grease", city_ids)
        self.assertIn("read_the_ways_list", city_ids)
        self.assertNotIn("read_the_ways_list", marsh_ids)

    def test_grip_bound_changes_grease_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_ways")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_oar_ways")["actions"])
        self.assertEqual(plain.state.location, "ways.grease")
        self.assertEqual(primed.state.location, "ways.grease")
        ...
        self.assertNotIn("grip_the_grease", plain_ids)
        self.assertIn("grip_the_grease", primed_ids)
        self.assertIn("grip_bound", primed.state.outcomes)

    def test_player_plain_language_launches_hull(self) -> None:
        ...
        self.assertIn("hull_launched", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the ways"):
    ...
    def test_i4_rejects_tampered_build_on_ways_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_hull_launched"))
