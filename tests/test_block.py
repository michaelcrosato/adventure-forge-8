"""Block Loft: bore-score-strop outcome, sheet divergence, flashing-house cross-effect."""
...
class BlockLoftTests(unittest.TestCase):
    @classmethod
    ...
    def test_block_stropped_trace_replays(self) -> None:
        trace = _trace("marsh_block_stropped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("block_stropped", result.state.outcomes)
        self.assertEqual(result.state.location, "block.strop")
        self.assertTrue(self.content.outcome_ready("block_stropped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_block")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_block")["actions"])
        self.assertEqual(marsh.state.location, "block.yard")
        ...
        self.assertIn("know_the_cheek", marsh_ids)
        self.assertNotIn("know_the_cheek", city_ids)
        self.assertIn("read_the_block_list", city_ids)
        self.assertNotIn("read_the_block_list", marsh_ids)

    def test_flash_dressed_changes_cheek_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_block")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_flash_block")["actions"])
        self.assertEqual(plain.state.location, "block.cheek")
        self.assertEqual(primed.state.location, "block.cheek")
        ...
        self.assertNotIn("dress_the_cheek", plain_ids)
        self.assertIn("dress_the_cheek", primed_ids)
        self.assertIn("flash_dressed", primed.state.outcomes)

    def test_player_plain_language_strops_block(self) -> None:
        ...
        self.assertIn("block_stropped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the blocks"):
    ...
    def test_i4_rejects_tampered_build_on_block_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_block_stropped"))
