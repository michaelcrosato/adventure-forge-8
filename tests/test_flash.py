"""Flashing House: roll-boss-dress outcome, sheet divergence, slate-yard cross-effect."""
...
class FlashingHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_flash_dressed_trace_replays(self) -> None:
        trace = _trace("marsh_flash_dressed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("flash_dressed", result.state.outcomes)
        self.assertEqual(result.state.location, "flash.apron")
        self.assertTrue(self.content.outcome_ready("flash_dressed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_flash")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_flash")["actions"])
        self.assertEqual(marsh.state.location, "flash.yard")
        ...
        self.assertIn("know_the_roll", marsh_ids)
        self.assertNotIn("know_the_roll", city_ids)
        self.assertIn("read_the_flash_list", city_ids)
        self.assertNotIn("read_the_flash_list", marsh_ids)

    def test_slate_lapped_changes_sheet_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_flash")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_slate_flash")["actions"])
        self.assertEqual(plain.state.location, "flash.sheet")
        self.assertEqual(primed.state.location, "flash.sheet")
        ...
        self.assertNotIn("lap_the_sheet", plain_ids)
        self.assertIn("lap_the_sheet", primed_ids)
        self.assertIn("slate_lapped", primed.state.outcomes)

    def test_player_plain_language_dresses_flash(self) -> None:
        ...
        self.assertIn("flash_dressed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the flash"):
    ...
    def test_i4_rejects_tampered_build_on_flash_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_flash_dressed"))
