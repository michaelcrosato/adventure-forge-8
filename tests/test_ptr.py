"""Pointer House: bevel-bed-dump outcome, sheet divergence, crutch-house cross-effect."""
...
class PointerHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_compound_dumped_trace_replays(self) -> None:
        trace = _trace("marsh_compound_dumped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("compound_dumped", result.state.outcomes)
        self.assertEqual(result.state.location, "ptr.compound")
        self.assertTrue(self.content.outcome_ready("compound_dumped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_ptr")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_ptr")["actions"])
        self.assertEqual(marsh.state.location, "ptr.yard")
        ...
        self.assertIn("know_the_pointer", marsh_ids)
        self.assertNotIn("know_the_pointer", city_ids)
        self.assertIn("read_the_pointer_list", city_ids)
        self.assertNotIn("read_the_pointer_list", marsh_ids)

    def test_crotch_spiked_changes_pointer_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_ptr")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_cru_ptr")["actions"])
        self.assertEqual(plain.state.location, "ptr.diagonal")
        self.assertEqual(primed.state.location, "ptr.diagonal")
        ...
        self.assertNotIn("spike_the_pointer", plain_ids)
        self.assertIn("spike_the_pointer", primed_ids)
        self.assertIn("crotch_spiked", primed.state.outcomes)

    def test_player_plain_language_dumps_compound(self) -> None:
        ...
        self.assertIn("compound_dumped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the pointer"):
    ...
    def test_i4_rejects_tampered_build_on_ptr_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_compound_dumped"))
