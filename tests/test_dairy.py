"""Dairy: set-wrap-salt outcome, sheet divergence, bakehouse cross-effect."""
...
class DairyTests(unittest.TestCase):
    @classmethod
    ...
    def test_wheel_salted_trace_replays(self) -> None:
        trace = _trace("marsh_wheel_salted")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("wheel_salted", result.state.outcomes)
        self.assertEqual(result.state.location, "dairy.shelf")
        self.assertTrue(self.content.outcome_ready("wheel_salted", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_dairy")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_dairy")["actions"])
        self.assertEqual(marsh.state.location, "dairy.yard")
        ...
        self.assertIn("know_the_rennet", marsh_ids)
        self.assertNotIn("know_the_rennet", city_ids)
        self.assertIn("read_the_dairy_list", city_ids)
        self.assertNotIn("read_the_dairy_list", marsh_ids)

    def test_loaf_drawn_changes_keeler_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_dairy")["actions"])
        scalded = replay(self.content, 1, "marsh_scout", _trace("cross_bake_dairy")["actions"])
        self.assertEqual(plain.state.location, "dairy.keel")
        self.assertEqual(scalded.state.location, "dairy.keel")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        scalded_ids = {a.id for a in enumerate_legal(scalded.state, self.content)}
        self.assertNotIn("scald_the_milk", plain_ids)
        self.assertIn("scald_the_milk", scalded_ids)
        self.assertIn("loaf_drawn", scalded.state.outcomes)

    def test_player_plain_language_salts_wheel(self) -> None:
        ...
        self.assertIn("wheel_salted", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the dairy"):
    ...
    def test_i4_rejects_tampered_build_on_dairy_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_wheel_salted"))
