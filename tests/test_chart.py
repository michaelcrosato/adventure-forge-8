"""Chart House: ruler-ink-seal outcome, sheet divergence, sounding-stage cross-effect."""
...
class ChartHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_rutter_sealed_trace_replays(self) -> None:
        trace = _trace("marsh_rutter_sealed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("rutter_sealed", result.state.outcomes)
        self.assertEqual(result.state.location, "chart.press")
        self.assertTrue(self.content.outcome_ready("rutter_sealed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_chart")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_chart")["actions"])
        self.assertEqual(marsh.state.location, "chart.yard")
        ...
        self.assertIn("know_the_coast_hand", marsh_ids)
        self.assertNotIn("know_the_coast_hand", city_ids)
        self.assertIn("read_the_rutter_list", city_ids)
        self.assertNotIn("read_the_rutter_list", marsh_ids)

    def test_lead_cast_changes_press_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_chart")["actions"])
        pricked = replay(self.content, 1, "marsh_scout", _trace("cross_lead_chart")["actions"])
        self.assertEqual(plain.state.location, "chart.press")
        self.assertEqual(pricked.state.location, "chart.press")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        pricked_ids = {a.id for a in enumerate_legal(pricked.state, self.content)}
        self.assertNotIn("prick_the_fathom", plain_ids)
        self.assertIn("prick_the_fathom", pricked_ids)
        self.assertIn("lead_cast", pricked.state.outcomes)

    def test_player_plain_language_seals_rutter(self) -> None:
        ...
        self.assertIn("rutter_sealed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to chart house"):
    ...
    def test_i4_rejects_tampered_build_on_chart_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_rutter_sealed"))
