"""Futtock House: cant-scarph-clench outcome, sheet divergence, binnacle-house cross-effect."""
...
class FuttockHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_belly_clenched_trace_replays(self) -> None:
        trace = _trace("marsh_belly_clenched")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("belly_clenched", result.state.outcomes)
        self.assertEqual(result.state.location, "futt.belly")
        self.assertTrue(self.content.outcome_ready("belly_clenched", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_futt")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_futt")["actions"])
        self.assertEqual(marsh.state.location, "futt.yard")
        ...
        self.assertIn("know_the_cant", marsh_ids)
        self.assertNotIn("know_the_cant", city_ids)
        self.assertIn("read_the_futtock_list", city_ids)
        self.assertNotIn("read_the_futtock_list", marsh_ids)

    def test_card_locked_changes_cant_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_futt")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_binn_futt")["actions"])
        self.assertEqual(plain.state.location, "futt.cant")
        self.assertEqual(primed.state.location, "futt.cant")
        ...
        self.assertNotIn("lock_the_cant", plain_ids)
        self.assertIn("lock_the_cant", primed_ids)
        self.assertIn("card_locked", primed.state.outcomes)

    def test_player_plain_language_clenches_belly(self) -> None:
        ...
        self.assertIn("belly_clenched", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the futtock"):
    ...
    def test_i4_rejects_tampered_build_on_futt_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_belly_clenched"))
