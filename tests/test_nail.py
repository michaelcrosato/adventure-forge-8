"""Nailery: snip-head-point outcome, sheet divergence, flax-house cross-effect."""
...
class NaileryTests(unittest.TestCase):
    @classmethod
    ...
    def test_nail_pointed_trace_replays(self) -> None:
        trace = _trace("marsh_nail_pointed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("nail_pointed", result.state.outcomes)
        self.assertEqual(result.state.location, "nail.shank")
        self.assertTrue(self.content.outcome_ready("nail_pointed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_nail")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_nail")["actions"])
        self.assertEqual(marsh.state.location, "nail.yard")
        ...
        self.assertIn("know_the_snip", marsh_ids)
        self.assertNotIn("know_the_snip", city_ids)
        self.assertIn("read_the_nail_list", city_ids)
        self.assertNotIn("read_the_nail_list", marsh_ids)

    def test_flax_spun_changes_snip_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_nail")["actions"])
        gripped = replay(self.content, 1, "marsh_scout", _trace("cross_flax_nail")["actions"])
        self.assertEqual(plain.state.location, "nail.snip")
        self.assertEqual(gripped.state.location, "nail.snip")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        gripped_ids = {a.id for a in enumerate_legal(gripped.state, self.content)}
        self.assertNotIn("flax_the_grip", plain_ids)
        self.assertIn("flax_the_grip", gripped_ids)
        self.assertIn("flax_spun", gripped.state.outcomes)

    def test_player_plain_language_points_nail(self) -> None:
        ...
        self.assertIn("nail_pointed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the nails"):
    ...
    def test_i4_rejects_tampered_build_on_nail_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_nail_pointed"))
