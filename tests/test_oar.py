"""Oar Loft: round-spoon-bind outcome, sheet divergence, block-loft cross-effect."""
...
class OarLoftTests(unittest.TestCase):
    @classmethod
    ...
    def test_grip_bound_trace_replays(self) -> None:
        trace = _trace("marsh_grip_bound")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("grip_bound", result.state.outcomes)
        self.assertEqual(result.state.location, "oar.grip")
        self.assertTrue(self.content.outcome_ready("grip_bound", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_oar")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_oar")["actions"])
        self.assertEqual(marsh.state.location, "oar.yard")
        ...
        self.assertIn("know_the_shaft", marsh_ids)
        self.assertNotIn("know_the_shaft", city_ids)
        self.assertIn("read_the_oar_list", city_ids)
        self.assertNotIn("read_the_oar_list", marsh_ids)

    def test_block_stropped_changes_shaft_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_oar")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_block_oar")["actions"])
        self.assertEqual(plain.state.location, "oar.shaft")
        self.assertEqual(primed.state.location, "oar.shaft")
        ...
        self.assertNotIn("strop_the_shaft", plain_ids)
        self.assertIn("strop_the_shaft", primed_ids)
        self.assertIn("block_stropped", primed.state.outcomes)

    def test_player_plain_language_binds_grip(self) -> None:
        ...
        self.assertIn("grip_bound", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the oars"):
    ...
    def test_i4_rejects_tampered_build_on_oar_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_grip_bound"))
