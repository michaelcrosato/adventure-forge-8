"""Kevel House: chamfer-mortise-clench outcome, sheet divergence, channel-house cross-effect."""
...
class KevelHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_kevel_clenched_trace_replays(self) -> None:
        trace = _trace("marsh_kevel_clenched")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("kevel_clenched", result.state.outcomes)
        self.assertEqual(result.state.location, "kev.clench")
        self.assertTrue(self.content.outcome_ready("kevel_clenched", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_kev")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_kev")["actions"])
        self.assertEqual(marsh.state.location, "kev.yard")
        ...
        self.assertIn("know_the_kevel", marsh_ids)
        self.assertNotIn("know_the_kevel", city_ids)
        self.assertIn("read_the_kevel_list", city_ids)
        self.assertNotIn("read_the_kevel_list", marsh_ids)

    def test_strap_set_changes_chamfer_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_kev")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_chan_kev")["actions"])
        self.assertEqual(plain.state.location, "kev.chamfer")
        self.assertEqual(primed.state.location, "kev.chamfer")
        ...
        self.assertNotIn("set_the_chamfer", plain_ids)
        self.assertIn("set_the_chamfer", primed_ids)
        self.assertIn("strap_set", primed.state.outcomes)

    def test_player_plain_language_clenches_kevel(self) -> None:
        ...
        self.assertIn("kevel_clenched", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the kevel"):
    ...
    def test_i4_rejects_tampered_build_on_kev_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_kevel_clenched"))
