"""Vinegar House: pitch-sour-cork outcome, sheet divergence, brew-house cross-effect."""
...
class VinegarHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_cruet_corked_trace_replays(self) -> None:
        trace = _trace("marsh_cruet_corked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("cruet_corked", result.state.outcomes)
        self.assertEqual(result.state.location, "acet.cruet")
        self.assertTrue(self.content.outcome_ready("cruet_corked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_acet")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_acet")["actions"])
        self.assertEqual(marsh.state.location, "acet.yard")
        ...
        self.assertIn("know_the_mother", marsh_ids)
        self.assertNotIn("know_the_mother", city_ids)
        self.assertIn("read_the_vinegar_list", city_ids)
        self.assertNotIn("read_the_vinegar_list", marsh_ids)

    def test_gyle_racked_changes_mother_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_acet")["actions"])
        aled = replay(self.content, 1, "marsh_scout", _trace("cross_brew_acet")["actions"])
        self.assertEqual(plain.state.location, "acet.mother")
        self.assertEqual(aled.state.location, "acet.mother")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        aled_ids = {a.id for a in enumerate_legal(aled.state, self.content)}
        self.assertNotIn("ale_the_mother", plain_ids)
        self.assertIn("ale_the_mother", aled_ids)
        self.assertIn("gyle_racked", aled.state.outcomes)

    def test_player_plain_language_corks_cruet(self) -> None:
        ...
        self.assertIn("cruet_corked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the vinegar"):
    ...
    def test_i4_rejects_tampered_build_on_acet_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_cruet_corked"))
