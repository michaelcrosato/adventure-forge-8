"""Gunwale House: snape-hance-dump outcome, sheet divergence, coaming-house cross-effect."""
...
class GunwaleHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_bolt_dumped_trace_replays(self) -> None:
        trace = _trace("marsh_bolt_dumped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("bolt_dumped", result.state.outcomes)
        self.assertEqual(result.state.location, "wale.dump")
        self.assertTrue(self.content.outcome_ready("bolt_dumped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_wale")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_wale")["actions"])
        self.assertEqual(marsh.state.location, "wale.yard")
        ...
        self.assertIn("know_the_covering", marsh_ids)
        self.assertNotIn("know_the_covering", city_ids)
        self.assertIn("read_the_gunwale_list", city_ids)
        self.assertNotIn("read_the_gunwale_list", marsh_ids)

    def test_coaming_coaked_changes_covering_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_wale")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_coam_wale")["actions"])
        self.assertEqual(plain.state.location, "wale.covering")
        self.assertEqual(primed.state.location, "wale.covering")
        ...
        self.assertNotIn("coak_the_covering", plain_ids)
        self.assertIn("coak_the_covering", primed_ids)
        self.assertIn("coaming_coaked", primed.state.outcomes)

    def test_player_plain_language_dumps_bolt(self) -> None:
        ...
        self.assertIn("bolt_dumped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the gunwale"):
    ...
    def test_i4_rejects_tampered_build_on_wale_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_bolt_dumped"))
