"""Transom House: scribe-fit-spike outcome, sheet divergence, cathead-house cross-effect."""
...
class TransomHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_stern_spiked_trace_replays(self) -> None:
        trace = _trace("marsh_stern_spiked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("stern_spiked", result.state.outcomes)
        self.assertEqual(result.state.location, "tran.stern")
        self.assertTrue(self.content.outcome_ready("stern_spiked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_tran")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_tran")["actions"])
        self.assertEqual(marsh.state.location, "tran.yard")
        ...
        self.assertIn("know_the_wing", marsh_ids)
        self.assertNotIn("know_the_wing", city_ids)
        self.assertIn("read_the_transom_list", city_ids)
        self.assertNotIn("read_the_transom_list", marsh_ids)

    def test_fluke_fished_changes_wing_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_tran")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_cath_tran")["actions"])
        self.assertEqual(plain.state.location, "tran.wing")
        self.assertEqual(primed.state.location, "tran.wing")
        ...
        self.assertNotIn("fish_the_wing", plain_ids)
        self.assertIn("fish_the_wing", primed_ids)
        self.assertIn("fluke_fished", primed.state.outcomes)

    def test_player_plain_language_spikes_stern(self) -> None:
        ...
        self.assertIn("stern_spiked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the transom"):
    ...
    def test_i4_rejects_tampered_build_on_tran_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_stern_spiked"))
