"""Newel House: turn-mortise-dowel outcome, sheet divergence, stair-house cross-effect."""
...
class NewelHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_finial_dowelled_trace_replays(self) -> None:
        trace = _trace("marsh_finial_dowelled")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("finial_dowelled", result.state.outcomes)
        self.assertEqual(result.state.location, "newel.dowel")
        self.assertTrue(self.content.outcome_ready("finial_dowelled", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_newel")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_newel")["actions"])
        self.assertEqual(marsh.state.location, "newel.yard")
        ...
        self.assertIn("know_the_blank", marsh_ids)
        self.assertNotIn("know_the_blank", city_ids)
        self.assertIn("read_the_newel_list", city_ids)
        self.assertNotIn("read_the_newel_list", marsh_ids)

    def test_riser_wedged_changes_blank_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_newel")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_stair_newel")["actions"])
        self.assertEqual(plain.state.location, "newel.blank")
        self.assertEqual(primed.state.location, "newel.blank")
        ...
        self.assertNotIn("wedge_the_blank", plain_ids)
        self.assertIn("wedge_the_blank", primed_ids)
        self.assertIn("riser_wedged", primed.state.outcomes)

    def test_player_plain_language_dowels_finial(self) -> None:
        ...
        self.assertIn("finial_dowelled", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the newel"):
    ...
    def test_i4_rejects_tampered_build_on_newel_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_finial_dowelled"))
