"""Fireback House: sand-pour-bed outcome, sheet divergence, flue-house cross-effect."""
...
class FirebackHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_back_bedded_trace_replays(self) -> None:
        trace = _trace("marsh_back_bedded")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("back_bedded", result.state.outcomes)
        self.assertEqual(result.state.location, "fb.bed")
        self.assertTrue(self.content.outcome_ready("back_bedded", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_fb")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_fb")["actions"])
        self.assertEqual(marsh.state.location, "fb.yard")
        ...
        self.assertIn("know_the_sand", marsh_ids)
        self.assertNotIn("know_the_sand", city_ids)
        self.assertIn("read_the_fireback_list", city_ids)
        self.assertNotIn("read_the_fireback_list", marsh_ids)

    def test_cowl_hung_changes_sand_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_fb")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_flue_fb")["actions"])
        self.assertEqual(plain.state.location, "fb.sand")
        self.assertEqual(primed.state.location, "fb.sand")
        ...
        self.assertNotIn("cowl_the_mould", plain_ids)
        self.assertIn("cowl_the_mould", primed_ids)
        self.assertIn("cowl_hung", primed.state.outcomes)

    def test_player_plain_language_beds_back(self) -> None:
        ...
        self.assertIn("back_bedded", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the fireback"):
    ...
    def test_i4_rejects_tampered_build_on_fb_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_back_bedded"))
