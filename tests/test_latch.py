"""Latch House: file-fit-throw outcome, sheet divergence, varnish-house cross-effect."""
...
class LatchHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_latch_thrown_trace_replays(self) -> None:
        trace = _trace("marsh_latch_thrown")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("latch_thrown", result.state.outcomes)
        self.assertEqual(result.state.location, "latch.hasp")
        self.assertTrue(self.content.outcome_ready("latch_thrown", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_latch")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_latch")["actions"])
        self.assertEqual(marsh.state.location, "latch.yard")
        ...
        self.assertIn("know_the_keep", marsh_ids)
        self.assertNotIn("know_the_keep", city_ids)
        self.assertIn("read_the_latch_list", city_ids)
        self.assertNotIn("read_the_latch_list", marsh_ids)

    def test_varnish_flowed_changes_keep_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_latch")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_varn_latch")["actions"])
        self.assertEqual(plain.state.location, "latch.keep")
        self.assertEqual(primed.state.location, "latch.keep")
        ...
        self.assertNotIn("varnish_the_keep", plain_ids)
        self.assertIn("varnish_the_keep", primed_ids)
        self.assertIn("varnish_flowed", primed.state.outcomes)

    def test_player_plain_language_throws_latch(self) -> None:
        ...
        self.assertIn("latch_thrown", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the latch"):
    ...
    def test_i4_rejects_tampered_build_on_latch_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_latch_thrown"))
