"""Gall House: crush-mix-cut outcome, sheet divergence, horn-lantern cross-effect."""
...
class GallHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_nib_cut_trace_replays(self) -> None:
        trace = _trace("marsh_nib_cut")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("nib_cut", result.state.outcomes)
        self.assertEqual(result.state.location, "gall.nib")
        self.assertTrue(self.content.outcome_ready("nib_cut", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_gall")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_gall")["actions"])
        self.assertEqual(marsh.state.location, "gall.yard")
        ...
        self.assertIn("know_the_gall_run", marsh_ids)
        self.assertNotIn("know_the_gall_run", city_ids)
        self.assertIn("read_the_ink_list", city_ids)
        self.assertNotIn("read_the_ink_list", marsh_ids)

    def test_lantern_hung_changes_crush_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_gall")["actions"])
        lit = replay(self.content, 1, "marsh_scout", _trace("cross_horn_gall")["actions"])
        self.assertEqual(plain.state.location, "gall.crush")
        self.assertEqual(lit.state.location, "gall.crush")
        ...
        self.assertNotIn("light_the_crush", plain_ids)
        self.assertIn("light_the_crush", lit_ids)
        self.assertIn("lantern_hung", lit.state.outcomes)

    def test_player_plain_language_cuts_nib(self) -> None:
        ...
        self.assertIn("nib_cut", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the galls"):
    ...
    def test_i4_rejects_tampered_build_on_gall_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_nib_cut"))
