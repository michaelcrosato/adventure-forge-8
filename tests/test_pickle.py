"""Pickle House: pack-and-lid outcome, sheet divergence, cooperage cross-effect."""
...
class PickleHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_pickle_lidded_trace_replays(self) -> None:
        trace = _trace("marsh_pickle_lidded")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("pickle_lidded", result.state.outcomes)
        self.assertEqual(result.state.location, "pickle.lid")
        self.assertTrue(self.content.outcome_ready("pickle_lidded", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_pickle")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_pickle")["actions"])
        self.assertEqual(marsh.state.location, "pickle.yard")
        ...
        self.assertIn("know_the_pickle_cut", marsh_ids)
        self.assertNotIn("know_the_pickle_cut", city_ids)
        self.assertIn("read_the_pickle_list", city_ids)
        self.assertNotIn("read_the_pickle_list", marsh_ids)

    def test_barrel_raised_changes_lid_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_pickle")["actions"])
        hooped = replay(self.content, 1, "marsh_scout", _trace("cross_coop_pickle")["actions"])
        self.assertEqual(plain.state.location, "pickle.lid")
        self.assertEqual(hooped.state.location, "pickle.lid")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        hooped_ids = {a.id for a in enumerate_legal(hooped.state, self.content)}
        self.assertNotIn("hoop_the_pickle", plain_ids)
        self.assertIn("hoop_the_pickle", hooped_ids)
        self.assertIn("barrel_raised", hooped.state.outcomes)

    def test_player_plain_language_lids_pickle(self) -> None:
        ...
        self.assertIn("pickle_lidded", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to pickle house"):
    ...
    def test_i4_rejects_tampered_build_on_pickle_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_pickle_lidded"))
