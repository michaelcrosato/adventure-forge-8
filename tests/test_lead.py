"""Sounding Stage: coil-mark-cast outcome, sheet divergence, sail-loft cross-effect."""
...
class SoundingStageTests(unittest.TestCase):
    @classmethod
    ...
    def test_lead_cast_trace_replays(self) -> None:
        trace = _trace("marsh_lead_cast")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("lead_cast", result.state.outcomes)
        self.assertEqual(result.state.location, "lead.cast")
        self.assertTrue(self.content.outcome_ready("lead_cast", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_lead")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_lead")["actions"])
        self.assertEqual(marsh.state.location, "lead.yard")
        ...
        self.assertIn("know_the_fathom_mark", marsh_ids)
        self.assertNotIn("know_the_fathom_mark", city_ids)
        self.assertIn("read_the_lead_list", city_ids)
        self.assertNotIn("read_the_lead_list", marsh_ids)

    def test_sail_hoisted_changes_cast_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_lead")["actions"])
        under = replay(self.content, 1, "marsh_scout", _trace("cross_sail_lead")["actions"])
        self.assertEqual(plain.state.location, "lead.cast")
        self.assertEqual(under.state.location, "lead.cast")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        under_ids = {a.id for a in enumerate_legal(under.state, self.content)}
        self.assertNotIn("sound_under_sail", plain_ids)
        self.assertIn("sound_under_sail", under_ids)
        self.assertIn("sail_hoisted", under.state.outcomes)

    def test_player_plain_language_casts_lead(self) -> None:
        ...
        self.assertIn("lead_cast", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the sounding"):
    ...
    def test_i4_rejects_tampered_build_on_lead_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_lead_cast"))
