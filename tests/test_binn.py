"""Binnacle House: true-fill-lock outcome, sheet divergence, scupper-house cross-effect."""
...
class BinnacleHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_card_locked_trace_replays(self) -> None:
        trace = _trace("marsh_card_locked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("card_locked", result.state.outcomes)
        self.assertEqual(result.state.location, "binn.card")
        self.assertTrue(self.content.outcome_ready("card_locked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_binn")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_binn")["actions"])
        self.assertEqual(marsh.state.location, "binn.yard")
        ...
        self.assertIn("know_the_hood", marsh_ids)
        self.assertNotIn("know_the_hood", city_ids)
        self.assertIn("read_the_binnacle_list", city_ids)
        self.assertNotIn("read_the_binnacle_list", marsh_ids)

    def test_mouth_plugged_changes_hood_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_binn")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_scup_binn")["actions"])
        self.assertEqual(plain.state.location, "binn.hood")
        self.assertEqual(primed.state.location, "binn.hood")
        ...
        self.assertNotIn("plug_the_hood", plain_ids)
        self.assertIn("plug_the_hood", primed_ids)
        self.assertIn("mouth_plugged", primed.state.outcomes)

    def test_player_plain_language_locks_card(self) -> None:
        ...
        self.assertIn("card_locked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the binnacle"):
    ...
    def test_i4_rejects_tampered_build_on_binn_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_card_locked"))
