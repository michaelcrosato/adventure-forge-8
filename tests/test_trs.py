"""Trestle-tree House: score-tenon-seize outcome, sheet divergence, spirketting-house cross-effect."""
...
class TrestleTreeHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_bib_seized_trace_replays(self) -> None:
        trace = _trace("marsh_bib_seized")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("bib_seized", result.state.outcomes)
        self.assertEqual(result.state.location, "trs.bib")
        self.assertTrue(self.content.outcome_ready("bib_seized", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_trs")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_trs")["actions"])
        self.assertEqual(marsh.state.location, "trs.yard")
        ...
        self.assertIn("know_the_trestle", marsh_ids)
        self.assertNotIn("know_the_trestle", city_ids)
        self.assertIn("read_the_trestle_list", city_ids)
        self.assertNotIn("read_the_trestle_list", marsh_ids)

    def test_spirket_clenched_changes_trestle_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_trs")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_spi_trs")["actions"])
        self.assertEqual(plain.state.location, "trs.trestle")
        self.assertEqual(primed.state.location, "trs.trestle")
        ...
        self.assertNotIn("clench_the_trestle", plain_ids)
        self.assertIn("clench_the_trestle", primed_ids)
        self.assertIn("spirket_clenched", primed.state.outcomes)

    def test_player_plain_language_seizes_bib(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to the trestle-trees",
            "go to trestle yard",
            "know the trestle",
            "go to the trestle",
            "score the trestle",
        ...
        self.assertIn("bib_seized", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the trestle-trees"):
    ...
    def test_i4_rejects_tampered_build_on_trs_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_bib_seized"))
