"""Stem House: hew-gripe-hang outcome, sheet divergence, mast-pond cross-effect."""
...
class StemHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_knee_hung_trace_replays(self) -> None:
        trace = _trace("marsh_knee_hung")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("knee_hung", result.state.outcomes)
        self.assertEqual(result.state.location, "stem.knee")
        self.assertTrue(self.content.outcome_ready("knee_hung", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_stem")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_stem")["actions"])
        self.assertEqual(marsh.state.location, "stem.yard")
        ...
        self.assertIn("know_the_hew", marsh_ids)
        self.assertNotIn("know_the_hew", city_ids)
        self.assertIn("read_the_stem_list", city_ids)
        self.assertNotIn("read_the_stem_list", marsh_ids)

    def test_partner_hooped_changes_hew_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_stem")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_mast_stem")["actions"])
        self.assertEqual(plain.state.location, "stem.hew")
        self.assertEqual(primed.state.location, "stem.hew")
        ...
        self.assertNotIn("hoop_the_hew", plain_ids)
        self.assertIn("hoop_the_hew", primed_ids)
        self.assertIn("partner_hooped", primed.state.outcomes)

    def test_player_plain_language_hangs_knee(self) -> None:
        ...
        self.assertIn("knee_hung", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the stem"):
    ...
    def test_i4_rejects_tampered_build_on_stem_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_knee_hung"))
