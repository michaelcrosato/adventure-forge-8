"""Crosstree House: square-spread-seize outcome, sheet divergence, futtock-house cross-effect."""
...
class CrosstreeHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_bolster_seized_trace_replays(self) -> None:
        trace = _trace("marsh_bolster_seized")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("bolster_seized", result.state.outcomes)
        self.assertEqual(result.state.location, "cros.bolster")
        self.assertTrue(self.content.outcome_ready("bolster_seized", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_cros")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_cros")["actions"])
        self.assertEqual(marsh.state.location, "cros.yard")
        ...
        self.assertIn("know_the_tree", marsh_ids)
        self.assertNotIn("know_the_tree", city_ids)
        self.assertIn("read_the_crosstree_list", city_ids)
        self.assertNotIn("read_the_crosstree_list", marsh_ids)

    def test_belly_clenched_changes_tree_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_cros")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_futt_cros")["actions"])
        self.assertEqual(plain.state.location, "cros.tree")
        self.assertEqual(primed.state.location, "cros.tree")
        ...
        self.assertNotIn("clench_the_tree", plain_ids)
        self.assertIn("clench_the_tree", primed_ids)
        self.assertIn("belly_clenched", primed.state.outcomes)

    def test_player_plain_language_seizes_bolster(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to the crosstrees",
            "go to crosstree yard",
            "know the tree",
            "go to the tree",
        ...
        self.assertIn("bolster_seized", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the crosstrees"):
    ...
    def test_i4_rejects_tampered_build_on_cros_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_bolster_seized"))
