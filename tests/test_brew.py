"""Brew House: charge-hop-rack outcome, sheet divergence, malt-house cross-effect."""
...
class BrewHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_gyle_racked_trace_replays(self) -> None:
        trace = _trace("marsh_gyle_racked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("gyle_racked", result.state.outcomes)
        self.assertEqual(result.state.location, "brew.gyle")
        self.assertTrue(self.content.outcome_ready("gyle_racked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_brew")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_brew")["actions"])
        self.assertEqual(marsh.state.location, "brew.yard")
        ...
        self.assertIn("know_the_grist", marsh_ids)
        self.assertNotIn("know_the_grist", city_ids)
        self.assertIn("read_the_brew_list", city_ids)
        self.assertNotIn("read_the_brew_list", marsh_ids)

    def test_malt_oasted_changes_grist_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_brew")["actions"])
        malted = replay(self.content, 1, "marsh_scout", _trace("cross_malt_brew")["actions"])
        self.assertEqual(plain.state.location, "brew.grist")
        self.assertEqual(malted.state.location, "brew.grist")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        malted_ids = {a.id for a in enumerate_legal(malted.state, self.content)}
        self.assertNotIn("malt_the_grist", plain_ids)
        self.assertIn("malt_the_grist", malted_ids)
        self.assertIn("malt_oasted", malted.state.outcomes)

    def test_player_plain_language_racks_gyle(self) -> None:
        ...
        self.assertIn("gyle_racked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the ale"):
    ...
    def test_i4_rejects_tampered_build_on_brew_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_gyle_racked"))
