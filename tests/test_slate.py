"""Slate Yard: scapple-punch-lap outcome, sheet divergence, tile-house cross-effect."""
...
class SlateYardTests(unittest.TestCase):
    @classmethod
    ...
    def test_slate_lapped_trace_replays(self) -> None:
        trace = _trace("marsh_slate_lapped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("slate_lapped", result.state.outcomes)
        self.assertEqual(result.state.location, "slate.lap")
        self.assertTrue(self.content.outcome_ready("slate_lapped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_slate")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_slate")["actions"])
        self.assertEqual(marsh.state.location, "slate.yard")
        ...
        self.assertIn("know_the_scapple", marsh_ids)
        self.assertNotIn("know_the_scapple", city_ids)
        self.assertIn("read_the_slate_list", city_ids)
        self.assertNotIn("read_the_slate_list", marsh_ids)

    def test_arris_nicked_changes_scapple_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_slate")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_tile_slate")["actions"])
        self.assertEqual(plain.state.location, "slate.scapple")
        self.assertEqual(primed.state.location, "slate.scapple")
        ...
        self.assertNotIn("arris_the_face", plain_ids)
        self.assertIn("arris_the_face", primed_ids)
        self.assertIn("arris_nicked", primed.state.outcomes)

    def test_player_plain_language_laps_slate(self) -> None:
        ...
        self.assertIn("slate_lapped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the slates"):
    ...
    def test_i4_rejects_tampered_build_on_slate_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_slate_lapped"))
