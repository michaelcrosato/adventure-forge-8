"""Floorboard House: shoot-tongue-secret outcome, sheet divergence, tread-house cross-effect."""
...
class FloorboardHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_nail_secreted_trace_replays(self) -> None:
        trace = _trace("marsh_nail_secreted")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("nail_secreted", result.state.outcomes)
        self.assertEqual(result.state.location, "floor.secret")
        self.assertTrue(self.content.outcome_ready("nail_secreted", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_floor")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_floor")["actions"])
        self.assertEqual(marsh.state.location, "floor.yard")
        ...
        self.assertIn("know_the_shot", marsh_ids)
        self.assertNotIn("know_the_shot", city_ids)
        self.assertIn("read_the_floor_list", city_ids)
        self.assertNotIn("read_the_floor_list", marsh_ids)

    def test_end_returned_changes_shot_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_floor")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_tread_floor")["actions"])
        self.assertEqual(plain.state.location, "floor.shot")
        self.assertEqual(primed.state.location, "floor.shot")
        ...
        self.assertNotIn("return_the_shot", plain_ids)
        self.assertIn("return_the_shot", primed_ids)
        self.assertIn("end_returned", primed.state.outcomes)

    def test_player_plain_language_secrets_nail(self) -> None:
        ...
        self.assertIn("nail_secreted", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the boards"):
    ...
    def test_i4_rejects_tampered_build_on_floor_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_nail_secreted"))
