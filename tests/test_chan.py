"""Channel House: score-bolt-set outcome, sheet divergence, boomkin-house cross-effect."""
...
class ChannelHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_strap_set_trace_replays(self) -> None:
        trace = _trace("marsh_strap_set")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("strap_set", result.state.outcomes)
        self.assertEqual(result.state.location, "chan.strap")
        self.assertTrue(self.content.outcome_ready("strap_set", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_chan")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_chan")["actions"])
        self.assertEqual(marsh.state.location, "chan.yard")
        ...
        self.assertIn("know_the_channel", marsh_ids)
        self.assertNotIn("know_the_channel", city_ids)
        self.assertIn("read_the_channel_list", city_ids)
        self.assertNotIn("read_the_channel_list", marsh_ids)

    def test_guy_seized_changes_face_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_chan")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_boom_chan")["actions"])
        self.assertEqual(plain.state.location, "chan.face")
        self.assertEqual(primed.state.location, "chan.face")
        ...
        self.assertNotIn("seize_the_face", plain_ids)
        self.assertIn("seize_the_face", primed_ids)
        self.assertIn("guy_seized", primed.state.outcomes)

    def test_player_plain_language_sets_strap(self) -> None:
        ...
        self.assertIn("strap_set", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the channel"):
    ...
    def test_i4_rejects_tampered_build_on_chan_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_strap_set"))
