"""Lath House: rive-prick-hair outcome, sheet divergence, joist-house cross-effect."""
...
class LathHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_coat_haired_trace_replays(self) -> None:
        trace = _trace("marsh_coat_haired")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("coat_haired", result.state.outcomes)
        self.assertEqual(result.state.location, "lath.hair")
        self.assertTrue(self.content.outcome_ready("coat_haired", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_lath")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_lath")["actions"])
        self.assertEqual(marsh.state.location, "lath.yard")
        ...
        self.assertIn("know_the_rive", marsh_ids)
        self.assertNotIn("know_the_rive", city_ids)
        self.assertIn("read_the_lath_list", city_ids)
        self.assertNotIn("read_the_lath_list", marsh_ids)

    def test_camber_crowned_changes_rive_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_lath")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_joist_lath")["actions"])
        self.assertEqual(plain.state.location, "lath.rive")
        self.assertEqual(primed.state.location, "lath.rive")
        ...
        self.assertNotIn("crown_the_rive", plain_ids)
        self.assertIn("crown_the_rive", primed_ids)
        self.assertIn("camber_crowned", primed.state.outcomes)

    def test_player_plain_language_hairs_coat(self) -> None:
        ...
        self.assertIn("coat_haired", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the laths"):
    ...
    def test_i4_rejects_tampered_build_on_lath_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_coat_haired"))
