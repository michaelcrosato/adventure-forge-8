"""Knighthead House: score-box-lash outcome, sheet divergence, kevel-house cross-effect."""
...
class KnightheadHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_knight_lashed_trace_replays(self) -> None:
        trace = _trace("marsh_knight_lashed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("knight_lashed", result.state.outcomes)
        self.assertEqual(result.state.location, "kni.lash")
        self.assertTrue(self.content.outcome_ready("knight_lashed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_kni")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_kni")["actions"])
        self.assertEqual(marsh.state.location, "kni.yard")
        ...
        self.assertIn("know_the_knight", marsh_ids)
        self.assertNotIn("know_the_knight", city_ids)
        self.assertIn("read_the_knighthead_list", city_ids)
        self.assertNotIn("read_the_knighthead_list", marsh_ids)

    def test_kevel_clenched_changes_knight_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_kni")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_kev_kni")["actions"])
        self.assertEqual(plain.state.location, "kni.knight")
        self.assertEqual(primed.state.location, "kni.knight")
        ...
        self.assertNotIn("clench_the_knight", plain_ids)
        self.assertIn("clench_the_knight", primed_ids)
        self.assertIn("kevel_clenched", primed.state.outcomes)

    def test_player_plain_language_lashes_knight(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to the knighthead",
            "go to knighthead yard",
            "know the knight",
            "go to the knight",
            "score the knight",
            "go to knighthead yard",
        ...
        self.assertIn("knight_lashed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the knighthead"):
    ...
    def test_i4_rejects_tampered_build_on_kni_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_knight_lashed"))
