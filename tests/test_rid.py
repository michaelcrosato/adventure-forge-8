"""Rider House: wring-seat-dump outcome, sheet divergence, dagger-knee-house cross-effect."""
...
class RiderHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_wring_dumped_trace_replays(self) -> None:
        trace = _trace("marsh_wring_dumped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("wring_dumped", result.state.outcomes)
        self.assertEqual(result.state.location, "rid.wring")
        self.assertTrue(self.content.outcome_ready("wring_dumped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_rid")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_rid")["actions"])
        self.assertEqual(marsh.state.location, "rid.yard")
        ...
        self.assertIn("know_the_rider", marsh_ids)
        self.assertNotIn("know_the_rider", city_ids)
        self.assertIn("read_the_rider_list", city_ids)
        self.assertNotIn("read_the_rider_list", marsh_ids)

    def test_sirmark_marked_changes_rider_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_rid")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_dag_rid")["actions"])
        self.assertEqual(plain.state.location, "rid.rider")
        self.assertEqual(primed.state.location, "rid.rider")
        ...
        self.assertNotIn("mark_the_rider", plain_ids)
        self.assertIn("mark_the_rider", primed_ids)
        self.assertIn("sirmark_marked", primed.state.outcomes)

    def test_player_plain_language_dumps_wring(self) -> None:
        ...
        self.assertIn("wring_dumped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the riders"):
    ...
    def test_i4_rejects_tampered_build_on_rid_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_wring_dumped"))
