"""Jam House: boil-skim-jar outcome, sheet divergence, pie-house cross-effect."""
...
class JamHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_jam_jarred_trace_replays(self) -> None:
        trace = _trace("marsh_jam_jarred")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("jam_jarred", result.state.outcomes)
        self.assertEqual(result.state.location, "jam.jar")
        self.assertTrue(self.content.outcome_ready("jam_jarred", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_jam")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_jam")["actions"])
        self.assertEqual(marsh.state.location, "jam.yard")
        ...
        self.assertIn("know_the_pulp", marsh_ids)
        self.assertNotIn("know_the_pulp", city_ids)
        self.assertIn("read_the_jam_list", city_ids)
        self.assertNotIn("read_the_jam_list", marsh_ids)

    def test_pie_crimped_changes_pulp_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_jam")["actions"])
        glazed = replay(self.content, 1, "marsh_scout", _trace("cross_pie_jam")["actions"])
        self.assertEqual(plain.state.location, "jam.pulp")
        self.assertEqual(glazed.state.location, "jam.pulp")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        glazed_ids = {a.id for a in enumerate_legal(glazed.state, self.content)}
        self.assertNotIn("pie_the_glaze", plain_ids)
        self.assertIn("pie_the_glaze", glazed_ids)
        self.assertIn("pie_crimped", glazed.state.outcomes)

    def test_player_plain_language_jars_jam(self) -> None:
        ...
        self.assertIn("jam_jarred", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the jam"):
    ...
    def test_i4_rejects_tampered_build_on_jam_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_jam_jarred"))
