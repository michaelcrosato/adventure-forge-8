"""Mantel House: bed-set-pin outcome, sheet divergence, chimney-house cross-effect."""
...
class MantelHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_mantel_pinned_trace_replays(self) -> None:
        trace = _trace("marsh_mantel_pinned")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("mantel_pinned", result.state.outcomes)
        self.assertEqual(result.state.location, "mant.over")
        self.assertTrue(self.content.outcome_ready("mantel_pinned", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_mant")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_mant")["actions"])
        self.assertEqual(marsh.state.location, "mant.yard")
        ...
        self.assertIn("know_the_lintel", marsh_ids)
        self.assertNotIn("know_the_lintel", city_ids)
        self.assertIn("read_the_mantel_list", city_ids)
        self.assertNotIn("read_the_mantel_list", marsh_ids)

    def test_breast_limed_changes_lintel_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_mant")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_chim_mant")["actions"])
        self.assertEqual(plain.state.location, "mant.lintel")
        self.assertEqual(primed.state.location, "mant.lintel")
        ...
        self.assertNotIn("lime_the_lintel", plain_ids)
        self.assertIn("lime_the_lintel", primed_ids)
        self.assertIn("breast_limed", primed.state.outcomes)

    def test_player_plain_language_pins_mantel(self) -> None:
        ...
        lines = [
            ...
            "take corbel pin",
            "go to mantel yard",
            "go to the overmantel",
            "pin the mantel",
        ]
        for line in lines:
            turn = session.apply_line(line)
            self.assertTrue(turn.accepted, line)
        self.assertIn("mantel_pinned", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the mantel"):
    ...
    def test_i4_rejects_tampered_build_on_mant_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_mantel_pinned"))
