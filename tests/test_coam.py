"""Coaming House: joggle-lodge-coak outcome, sheet divergence, waterway-house cross-effect."""
...
class CoamingHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_coaming_coaked_trace_replays(self) -> None:
        trace = _trace("marsh_coaming_coaked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("coaming_coaked", result.state.outcomes)
        self.assertEqual(result.state.location, "coam.coak")
        self.assertTrue(self.content.outcome_ready("coaming_coaked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_coam")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_coam")["actions"])
        self.assertEqual(marsh.state.location, "coam.yard")
        ...
        self.assertIn("know_the_joggle", marsh_ids)
        self.assertNotIn("know_the_joggle", city_ids)
        self.assertIn("read_the_coaming_list", city_ids)
        self.assertNotIn("read_the_coaming_list", marsh_ids)

    def test_waterway_paid_changes_joggle_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_coam")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_wtr_coam")["actions"])
        self.assertEqual(plain.state.location, "coam.joggle")
        self.assertEqual(primed.state.location, "coam.joggle")
        ...
        self.assertNotIn("pay_the_joggle", plain_ids)
        self.assertIn("pay_the_joggle", primed_ids)
        self.assertIn("waterway_paid", primed.state.outcomes)

    def test_player_plain_language_coaks_coaming(self) -> None:
        ...
        self.assertIn("coaming_coaked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the coaming"):
    ...
    def test_i4_rejects_tampered_build_on_coam_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_coaming_coaked"))
