"""Rushlight House: peel-and-bind outcome, sheet divergence, decoy-pond cross-effect."""
...
class RushlightHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_lights_bound_trace_replays(self) -> None:
        trace = _trace("marsh_lights_bound")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("lights_bound", result.state.outcomes)
        self.assertEqual(result.state.location, "rush.bind")
        self.assertTrue(self.content.outcome_ready("lights_bound", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_rush")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_rush")["actions"])
        self.assertEqual(marsh.state.location, "rush.yard")
        ...
        self.assertIn("know_the_rush_peel", marsh_ids)
        self.assertNotIn("know_the_rush_peel", city_ids)
        self.assertIn("read_the_rush_list", city_ids)
        self.assertNotIn("read_the_rush_list", marsh_ids)

    def test_fowl_taken_changes_bind_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_rush")["actions"])
        tallowed = replay(self.content, 1, "marsh_scout", _trace("cross_decoy_rush")["actions"])
        self.assertEqual(plain.state.location, "rush.bind")
        self.assertEqual(tallowed.state.location, "rush.bind")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        tallowed_ids = {a.id for a in enumerate_legal(tallowed.state, self.content)}
        self.assertNotIn("tallow_the_rush", plain_ids)
        self.assertIn("tallow_the_rush", tallowed_ids)
        self.assertIn("fowl_taken", tallowed.state.outcomes)

    def test_player_plain_language_binds_lights(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        lines = [
            "go to market",
            "go to causeway",
            "go to rushlight house",
            "go to rush yard",
            "know the rush peel",
            "go to the peel",
            "peel the rush",
        ...
        self.assertIn("lights_bound", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to rushlight house"):
    ...
    def test_i4_rejects_tampered_build_on_rush_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_lights_bound"))
