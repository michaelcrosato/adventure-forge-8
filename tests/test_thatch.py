"""Thatch Croft: cut-bind-set outcome, sheet divergence, mason-yard cross-effect."""
...
class ThatchCroftTests(unittest.TestCase):
    @classmethod
    ...
    def test_roof_set_trace_replays(self) -> None:
        trace = _trace("marsh_roof_set")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("roof_set", result.state.outcomes)
        self.assertEqual(result.state.location, "thatch.ridge")
        self.assertTrue(self.content.outcome_ready("roof_set", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_thatch")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_thatch")["actions"])
        self.assertEqual(marsh.state.location, "thatch.yard")
        ...
        self.assertIn("know_the_yealm", marsh_ids)
        self.assertNotIn("know_the_yealm", city_ids)
        self.assertIn("read_the_thatch_list", city_ids)
        self.assertNotIn("read_the_thatch_list", marsh_ids)

    def test_joint_pointed_changes_ridge_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_thatch")["actions"])
        seated = replay(self.content, 1, "marsh_scout", _trace("cross_mason_thatch")["actions"])
        self.assertEqual(plain.state.location, "thatch.ridge")
        self.assertEqual(seated.state.location, "thatch.ridge")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        seated_ids = {a.id for a in enumerate_legal(seated.state, self.content)}
        self.assertNotIn("set_on_stone", plain_ids)
        self.assertIn("set_on_stone", seated_ids)
        self.assertIn("joint_pointed", seated.state.outcomes)

    def test_player_plain_language_sets_roof(self) -> None:
        ...
        self.assertIn("roof_set", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the thatch"):
    ...
    def test_i4_rejects_tampered_build_on_thatch_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_roof_set"))
