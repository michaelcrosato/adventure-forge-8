"""Sill House: bed-kerf-seat outcome, sheet divergence, stay-house cross-effect."""
...
class SillHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_stool_seated_trace_replays(self) -> None:
        trace = _trace("marsh_stool_seated")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("stool_seated", result.state.outcomes)
        self.assertEqual(result.state.location, "sill.stool")
        self.assertTrue(self.content.outcome_ready("stool_seated", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_sill")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_sill")["actions"])
        self.assertEqual(marsh.state.location, "sill.yard")
        ...
        self.assertIn("know_the_sill", marsh_ids)
        self.assertNotIn("know_the_sill", city_ids)
        self.assertIn("read_the_sill_list", city_ids)
        self.assertNotIn("read_the_sill_list", marsh_ids)

    def test_casement_stayed_changes_bed_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_sill")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_stay_sill")["actions"])
        self.assertEqual(plain.state.location, "sill.bed")
        self.assertEqual(primed.state.location, "sill.bed")
        ...
        self.assertNotIn("stay_the_sill", plain_ids)
        self.assertIn("stay_the_sill", primed_ids)
        self.assertIn("casement_stayed", primed.state.outcomes)

    def test_player_plain_language_seats_stool(self) -> None:
        ...
        self.assertIn("stool_seated", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the sill"):
    ...
    def test_i4_rejects_tampered_build_on_sill_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_stool_seated"))
