"""Tile House: drape-pallet-nick outcome, sheet divergence, brick-yard cross-effect."""
...
class TileHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_arris_nicked_trace_replays(self) -> None:
        trace = _trace("marsh_arris_nicked")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("arris_nicked", result.state.outcomes)
        self.assertEqual(result.state.location, "tile.arris")
        self.assertTrue(self.content.outcome_ready("arris_nicked", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_tile")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_tile")["actions"])
        self.assertEqual(marsh.state.location, "tile.yard")
        ...
        self.assertIn("know_the_horse", marsh_ids)
        self.assertNotIn("know_the_horse", city_ids)
        self.assertIn("read_the_tile_list", city_ids)
        self.assertNotIn("read_the_tile_list", marsh_ids)

    def test_hack_set_changes_horse_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_tile")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_brick_tile")["actions"])
        self.assertEqual(plain.state.location, "tile.horse")
        self.assertEqual(primed.state.location, "tile.horse")
        ...
        self.assertNotIn("hack_the_horse", plain_ids)
        self.assertIn("hack_the_horse", primed_ids)
        self.assertIn("hack_set", primed.state.outcomes)

    def test_player_plain_language_nicks_arris(self) -> None:
        ...
        self.assertIn("arris_nicked", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the tiles"):
    ...
    def test_i4_rejects_tampered_build_on_tile_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_arris_nicked"))
