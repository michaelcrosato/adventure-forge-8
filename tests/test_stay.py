"""Stay House: slot-rivet-peg outcome, sheet divergence, hinge-house cross-effect."""
...
class StayHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_casement_stayed_trace_replays(self) -> None:
        trace = _trace("marsh_casement_stayed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("casement_stayed", result.state.outcomes)
        self.assertEqual(result.state.location, "stay.eye")
        self.assertTrue(self.content.outcome_ready("casement_stayed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_stay")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_stay")["actions"])
        self.assertEqual(marsh.state.location, "stay.yard")
        ...
        self.assertIn("know_the_slot", marsh_ids)
        self.assertNotIn("know_the_slot", city_ids)
        self.assertIn("read_the_stay_list", city_ids)
        self.assertNotIn("read_the_stay_list", marsh_ids)

    def test_gudgeon_shipped_changes_slot_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_stay")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_hinge_stay")["actions"])
        self.assertEqual(plain.state.location, "stay.slot")
        self.assertEqual(primed.state.location, "stay.slot")
        ...
        self.assertNotIn("hinge_the_slot", plain_ids)
        self.assertIn("hinge_the_slot", primed_ids)
        self.assertIn("gudgeon_shipped", primed.state.outcomes)

    def test_player_plain_language_pegs_stay(self) -> None:
        ...
        self.assertIn("casement_stayed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the stay"):
    ...
    def test_i4_rejects_tampered_build_on_stay_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_casement_stayed"))
