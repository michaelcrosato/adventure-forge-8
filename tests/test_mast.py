"""Mast Pond: sink-range-hoop outcome, sheet divergence, deadwood cross-effect."""
...
class MastPondTests(unittest.TestCase):
    @classmethod
    ...
    def test_partner_hooped_trace_replays(self) -> None:
        trace = _trace("marsh_partner_hooped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("partner_hooped", result.state.outcomes)
        self.assertEqual(result.state.location, "mast.partner")
        self.assertTrue(self.content.outcome_ready("partner_hooped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_mast")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_mast")["actions"])
        self.assertEqual(marsh.state.location, "mast.yard")
        ...
        self.assertIn("know_the_sink", marsh_ids)
        self.assertNotIn("know_the_sink", city_ids)
        self.assertIn("read_the_mast_list", city_ids)
        self.assertNotIn("read_the_mast_list", marsh_ids)

    def test_hog_bolted_changes_sink_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_mast")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_dead_mast")["actions"])
        self.assertEqual(plain.state.location, "mast.sink")
        self.assertEqual(primed.state.location, "mast.sink")
        ...
        self.assertNotIn("bolt_the_sink", plain_ids)
        self.assertIn("bolt_the_sink", primed_ids)
        self.assertIn("hog_bolted", primed.state.outcomes)

    def test_player_plain_language_hoops_partner(self) -> None:
        ...
        self.assertIn("partner_hooped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the masts"):
    ...
    def test_i4_rejects_tampered_build_on_mast_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_partner_hooped"))
