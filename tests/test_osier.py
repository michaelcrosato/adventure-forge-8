"""Osier Holt: cut-brake-weave outcome, sheet divergence, rag-mill cross-effect."""
...
class OsierHoltTests(unittest.TestCase):
    @classmethod
    ...
    def test_frail_woven_trace_replays(self) -> None:
        trace = _trace("marsh_frail_woven")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("frail_woven", result.state.outcomes)
        self.assertEqual(result.state.location, "osier.frame")
        self.assertTrue(self.content.outcome_ready("frail_woven", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_osier")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_osier")["actions"])
        self.assertEqual(marsh.state.location, "osier.yard")
        ...
        self.assertIn("know_the_holt_cut", marsh_ids)
        self.assertNotIn("know_the_holt_cut", city_ids)
        self.assertIn("read_the_osier_list", city_ids)
        self.assertNotIn("read_the_osier_list", marsh_ids)

    def test_paper_laid_changes_holt_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_osier")["actions"])
        wrapped = replay(self.content, 1, "marsh_scout", _trace("cross_rag_osier")["actions"])
        self.assertEqual(plain.state.location, "osier.holt")
        self.assertEqual(wrapped.state.location, "osier.holt")
        ...
        self.assertNotIn("wrap_the_holt", plain_ids)
        self.assertIn("wrap_the_holt", wrapped_ids)
        self.assertIn("paper_laid", wrapped.state.outcomes)

    def test_player_plain_language_weaves_frail(self) -> None:
        ...
        self.assertIn("frail_woven", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the osiers"):
    ...
    def test_i4_rejects_tampered_build_on_osier_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_frail_woven"))
