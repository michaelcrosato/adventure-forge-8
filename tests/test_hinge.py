"""Hinge House: form-drift-ship outcome, sheet divergence, latch-house cross-effect."""
...
class HingeHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_gudgeon_shipped_trace_replays(self) -> None:
        trace = _trace("marsh_gudgeon_shipped")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("gudgeon_shipped", result.state.outcomes)
        self.assertEqual(result.state.location, "hinge.gudgeon")
        self.assertTrue(self.content.outcome_ready("gudgeon_shipped", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_hinge")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_hinge")["actions"])
        self.assertEqual(marsh.state.location, "hinge.yard")
        ...
        self.assertIn("know_the_knuckle", marsh_ids)
        self.assertNotIn("know_the_knuckle", city_ids)
        self.assertIn("read_the_hinge_list", city_ids)
        self.assertNotIn("read_the_hinge_list", marsh_ids)

    def test_latch_thrown_changes_knuckle_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_hinge")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_latch_hinge")["actions"])
        self.assertEqual(plain.state.location, "hinge.knuckle")
        self.assertEqual(primed.state.location, "hinge.knuckle")
        ...
        self.assertNotIn("latch_the_knuckle", plain_ids)
        self.assertIn("latch_the_knuckle", primed_ids)
        self.assertIn("latch_thrown", primed.state.outcomes)

    def test_player_plain_language_ships_gudgeon(self) -> None:
        ...
        self.assertIn("gudgeon_shipped", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the hinge"):
    ...
    def test_i4_rejects_tampered_build_on_hinge_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_gudgeon_shipped"))
