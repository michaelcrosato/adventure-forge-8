"""Jeweler: beat-seat-close outcome, sheet divergence, gilder's-loft cross-effect."""
...
class JewelerTests(unittest.TestCase):
    @classmethod
    ...
    def test_collet_closed_trace_replays(self) -> None:
        trace = _trace("marsh_collet_closed")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("collet_closed", result.state.outcomes)
        self.assertEqual(result.state.location, "gem.collet")
        self.assertTrue(self.content.outcome_ready("collet_closed", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_gem")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_gem")["actions"])
        self.assertEqual(marsh.state.location, "gem.yard")
        ...
        self.assertIn("know_the_foil", marsh_ids)
        self.assertNotIn("know_the_foil", city_ids)
        self.assertIn("read_the_gem_list", city_ids)
        self.assertNotIn("read_the_gem_list", marsh_ids)

    def test_plate_burnished_changes_foil_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_gem")["actions"])
        gilted = replay(self.content, 1, "marsh_scout", _trace("cross_gilt_gem")["actions"])
        self.assertEqual(plain.state.location, "gem.foil")
        self.assertEqual(gilted.state.location, "gem.foil")
        plain_ids = {a.id for a in enumerate_legal(plain.state, self.content)}
        gilted_ids = {a.id for a in enumerate_legal(gilted.state, self.content)}
        self.assertNotIn("gilt_the_foil", plain_ids)
        self.assertIn("gilt_the_foil", gilted_ids)
        self.assertIn("plate_burnished", gilted.state.outcomes)

    def test_player_plain_language_closes_collet(self) -> None:
        ...
        self.assertIn("collet_closed", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the jeweler"):
    ...
    def test_i4_rejects_tampered_build_on_gem_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_collet_closed"))
