"""Tread House: mark-nosing-return outcome, sheet divergence, baluster-house cross-effect."""
...
class TreadHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_end_returned_trace_replays(self) -> None:
        trace = _trace("marsh_end_returned")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("end_returned", result.state.outcomes)
        self.assertEqual(result.state.location, "tread.end")
        self.assertTrue(self.content.outcome_ready("end_returned", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_tread")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_tread")["actions"])
        self.assertEqual(marsh.state.location, "tread.yard")
        ...
        self.assertIn("know_the_going", marsh_ids)
        self.assertNotIn("know_the_going", city_ids)
        self.assertIn("read_the_tread_list", city_ids)
        self.assertNotIn("read_the_tread_list", marsh_ids)

    def test_neck_shouldered_changes_going_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_tread")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_bal_tread")["actions"])
        self.assertEqual(plain.state.location, "tread.going")
        self.assertEqual(primed.state.location, "tread.going")
        ...
        self.assertNotIn("shoulder_the_going", plain_ids)
        self.assertIn("shoulder_the_going", primed_ids)
        self.assertIn("neck_shouldered", primed.state.outcomes)

    def test_player_plain_language_returns_end(self) -> None:
        ...
        self.assertIn("end_returned", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the treads"):
    ...
    def test_i4_rejects_tampered_build_on_tread_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_end_returned"))
