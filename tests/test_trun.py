"""Treenail House: shave-auger-drive outcome, sheet divergence, mould-loft cross-effect."""
...
class TreenailHouseTests(unittest.TestCase):
    @classmethod
    ...
    def test_trunnel_driven_trace_replays(self) -> None:
        trace = _trace("marsh_trunnel_driven")
        result = replay(self.content, trace["seed"], trace["sheet"], trace["actions"])
        self.assertIn("trunnel_driven", result.state.outcomes)
        self.assertEqual(result.state.location, "trun.maul")
        self.assertTrue(self.content.outcome_ready("trunnel_driven", result.state))
    ...
    def test_yard_sheet_divergence(self) -> None:
        marsh = replay(self.content, 1, "marsh_scout", _trace("divergence_marsh_trun")["actions"])
        city = replay(self.content, 1, "city_oath", _trace("divergence_city_trun")["actions"])
        self.assertEqual(marsh.state.location, "trun.yard")
        ...
        self.assertIn("know_the_billet", marsh_ids)
        self.assertNotIn("know_the_billet", city_ids)
        self.assertIn("read_the_trunnel_list", city_ids)
        self.assertNotIn("read_the_trunnel_list", marsh_ids)

    def test_station_bevelled_changes_billet_verbs(self) -> None:
        plain = replay(self.content, 1, "marsh_scout", _trace("cross_plain_trun")["actions"])
        primed = replay(self.content, 1, "marsh_scout", _trace("cross_offs_trun")["actions"])
        self.assertEqual(plain.state.location, "trun.billet")
        self.assertEqual(primed.state.location, "trun.billet")
        ...
        self.assertNotIn("bevel_the_billet", plain_ids)
        self.assertIn("bevel_the_billet", primed_ids)
        self.assertIn("station_bevelled", primed.state.outcomes)

    def test_player_plain_language_drives_trunnel(self) -> None:
        ...
        self.assertIn("trunnel_driven", session.state.outcomes)
    ...
    def test_unmapped_text_still_noop_in_yard(self) -> None:
        session = PlaySession.start(self.content, 1, "marsh_scout")
        for line in ("go to market", "go to causeway", "go to the trunnels"):
    ...
    def test_i4_rejects_tampered_build_on_trun_trace(self) -> None:
        trace = copy.deepcopy(_trace("marsh_trunnel_driven"))
