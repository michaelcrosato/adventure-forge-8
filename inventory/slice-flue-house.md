...
`flue.path`, `flue.yard`, `flue.parge`, `flue.throat`, `flue.cowl`. Linked from `ashfen.causeway` and `mant.path`. Avoids `Go to the pot` (mustard) and `Go to the cap` (dado).

## Outcome

`cowl_hung` — flag `cowl_hung`. Witness: `traces/marsh_cowl_hung.json`.

## Sheet / deed gates

Same scene `flue.yard`: marshborn/hunt `know_the_parge`; letters `read_the_flue_list`.

## Cross-effect

Pinned mantel (`mantel_pinned`) unlocks `pin_the_parge` at the parge bench. Proven by `cross_mant_flue` vs `cross_plain_flue`.
