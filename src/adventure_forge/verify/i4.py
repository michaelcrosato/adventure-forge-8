...
def check_i4(content: Content, traces: list[dict]) -> dict:
    ...
    # Every remaining shipped outcome needs a primary walkthrough.
    missing_outcomes = []
    outcome_traces: dict[str, dict] = {}
    for oid in content.outcomes:
        found = [t for t in traces if t.get("outcome") == oid]
        if not found:
            missing_outcomes.append(oid)
        else:
            outcome_traces[oid] = found[0]
    if missing_outcomes:
        raise AssertionError(f"I4 missing outcome witnesses {missing_outcomes}")

    for trace in traces:
        accept_trace(content, trace)

    fingerprints: dict[str, str] = {}
    for oid, spec in outcome_traces.items():
        result = replay(content, spec["seed"], spec["sheet"], spec["actions"])
        if oid not in result.state.outcomes:
            raise AssertionError(f"{oid} predicate failed")
        if not content.outcome_ready(oid, result.state):
            raise AssertionError(f"{oid} acceptor did not fire")
        fingerprints[oid] = result.fingerprint
    ...
    pil_m = by_id["divergence_marsh_pil"]
    pil_c = by_id["divergence_city_pil"]
    pil_m_ids, pil_c_ids = _diverge_legal(content, pil_m, pil_c)
    if "know_the_pillar" not in pil_m_ids:
        raise AssertionError("marsh_scout missing know_the_pillar")
    if "read_the_pillar_list" not in pil_c_ids:
        raise AssertionError("city_oath missing read_the_pillar_list")
    if "know_the_pillar" in pil_c_ids or "read_the_pillar_list" in pil_m_ids:
        raise AssertionError("pil sheet verbs leaked across sheets")
